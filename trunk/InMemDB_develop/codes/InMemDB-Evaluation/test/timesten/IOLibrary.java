...
public class IOLibrary {
	 ...
	 * 
	 * @param args
	 *            String array of arguments to parse
	 * @param usage
	 *            Print this usage String on errors during argument parsing
	 * 
	...
	public String getUsageString(String programName) {
		String usageString = "\n" + "Usage: " + programName
				...
				+ "  connstr      connection string, e.g. \"sampledb_1121\"\n"
				+ "  Usage examples:\n" + "	  java " + programName + "\n"
				+ "	  java " + programName + " -c\n" + "	  java " + programName
				+ " mydsn_name\n"
				+ "Program defaults if parameters not specified:\n"
				+ "connstr=sampledb_1121, -d for direct-linked";
	...
	public boolean parseOpts(String[] args, String usage) {
		...
		boolean argsOk;
		String arg;
		for (i = 0, argsOk = true, arg = null; argsOk && (i < args.length)
				&& ((arg = args[i]).startsWith("-")); i++) {
		...
		if (opt_connstr.equals("")) {
			// The default DSN is sampledb[CS]_1121
			if (opt_doClient) {
				// opt_connstr = "sampledbCS_1121";
				opt_connstr = tt_version.sample_DSN_CS;
			} else {
				// opt_connstr = "sampledb_1121";
	...
	public static void main(String args[]) {
		...
		String correctargs[] = { "-c", "sampledb_1121" };
		...
		if (myLib.parseOpts(correctargs, usageStringTwo)) {
			System.out.println(correctargs[0] + " " + correctargs[1]
					+ " is correct");
			testOnePassed = true;
		} else {
			System.out.println(correctargs[0] + " " + correctargs[1]
