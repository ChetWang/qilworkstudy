	...
	public void insertTable(int status, String sendNumber, String recvNumber,
		...
		try {
			...
			if (replyFlag) {
				ack = 1;
				if (!msgID.equalsIgnoreCase("")) {
					db.prepareStatement(Util.getDialect()
				...
				if (replyNO.length() > 0) {
					...
					db
							.prepareStatement("select * from application where spNO=?");
					db.setString(1, replyNO);
					ResultSet rs1 = db.executeQuery();
					if (rs1.next()) {
						src_appID = rs1.getString("appID");
			...
			if (src_appID.length() > 0) {
				if (src_reply.length() == 0
					...
					outMsg.put("APP", src_appID);
					outMsg.put("ID", src_appSerialNO);
					outMsg.put("MSG", sendNumber + ":" + content);
					outMsg.put("UMS_TO", src_reply);
					...
					process.process(outMsg, 1, serial);
					insertSQL = insertSQL
							.append(
									"insert into in_ok(BatchNo,SerialNo,retCode,errMsg,statusFlag,AppId,AppSerialNo,MediaId,SendId,RecvId,SubmitDate,SubmitTime,finishDate,finishTime,content,ack,reply,msgType)")
							.append(" values('").append(currentTime).append(
									"',").append(Integer.parseInt(serial))
							.append(",'','',0,'").append(src_appID).append(
									"','").append(src_appSerialNO)
							.append("','").append(mediaId).append("','")
							.append(sendNumber).append("','")
							.append(src_sendID).append("','")
							.append(submitDate).append("','")
							.append(submitTime).append("','','','").append(
									content).append("',").append(ack).append(
									",'").append(src_reply).append("',")
							.append(data_code).append(")");
					Res.log(Res.DEBUG, "insertSQL=" + insertSQL);
					db.executeUpdate(insertSQL.toString());
					Res.log(Res.DEBUG, "插入一条信息到数据库成功！");
				}
