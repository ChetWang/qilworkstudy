...
public class NCIThumbnailPanel extends EditorPanel {
    ...
    public NCIThumbnailPanel(Editor editor) {
    	super(editor);
        initComponents();
    ...
    public NCIThumbnailPanel(int type,Editor editor) {
    	super(editor);
        switch (type) {
        ...
        // 这个一定要加上，限制JSvgCanvas的大小，使用preferredsize不能严格控制
        svgCanvas.setRecenterOnResize(true);
        svgCanvas.setBackground(Constants.THUMBNAIL_BACKGROUND);
    }
     ...
     * @param mouseSatus
     *            鼠标状态，参见Constants类
     * @see com.nci.svg.util.Constants
     */
     private void mouseAction(int mouseStatus, MouseEvent e) {
        if (mouseStatus == Constants.MOUSE_PRESSED || mouseStatus == Constants.MOUSE_RELEASED) {
            editor.getGraphUnitManager().setMouseStatus(mouseStatus);
            if (mouseStatus == Constants.MOUSE_PRESSED) {
                ...
                if (type != null && type.equals("true")) { //连接线要特殊处理
                    PathShape path = new PathShape(editor);
                    path.enableConnectedPath(true);
                    path.notifyDrawingMode();
                } else {
                    GraphUnitImageShape guImageShape = new GraphUnitImageShape(editor);
    ...
    private void setCursorShape(int mouseStatus) {
        switch (mouseStatus) {
            ...
            case Constants.MOUSE_ENTERED:
                if (editor.getGraphUnitManager().getMouseStatus() != Constants.MOUSE_PRESSED)
