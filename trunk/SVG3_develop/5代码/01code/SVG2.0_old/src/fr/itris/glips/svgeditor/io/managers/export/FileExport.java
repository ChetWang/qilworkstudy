...
import fr.itris.glips.svgeditor.io.managers.dialog.ExportFileFilter;
import fr.itris.glips.svgeditor.io.managers.dialog.FileChooserDialog;
import fr.itris.glips.svgeditor.io.managers.export.handler.Export;
...
public class FileExport {
	 ...
	 */
//	public static int[] formats = { JPG_FORMAT, PNG_FORMAT, BMP_FORMAT,
//			PDF_FORMAT };
	public static int[] formats = { JPG_FORMAT, PNG_FORMAT, BMP_FORMAT,
		};
...
//	public static String[] prefixLabels = { "JPG", "PNG", "BMP", "PDF" };
	public static String[] prefixLabels = { "JPG", "PNG", "BMP" };
	 ...
	 */
	private FileChooserDialog[] fileChooserDialogs = new FileChooserDialog[formats.length];
	...
	protected void initialize() {
		...
		for (int i = 0; i < formats.length; i++) {
			...
			if (getEditor().getParent() instanceof Frame) {

				fileChooserDialogs[i] = new FileChooserDialog((Frame) getEditor()
						.getParent(), FileChooserDialog.EXPORT_FILE_MODE,getEditor());

			} else if (getEditor().getParent() instanceof JDialog) {

				fileChooserDialogs[i] = new FileChooserDialog((JDialog) getEditor()
						.getParent(), FileChooserDialog.EXPORT_FILE_MODE,getEditor());
			}
	...
	/**
	 * exports an image representation of the provided handle ; the type of the
	 * export is defined by the provided index in the format array
	 * 
	 * @param handle
	 *            a handle
	 * @param index
	 *            an index in the format array
	...
	public void export(final SVGHandle handle, final int index,
		...
		if (getEditor().getSelectionManager().getSelectionMode() == SelectionInfoManager.SCREEN_CAST_MODE) {
			Export exportManager = exportHandlers[index];
			exportManager.export(relativeComponent, handle.getCanvas()
					.getDocument(), new File(Constants.NCI_SVG_CACHE_CLIP_TEMP_CACHE_DIR+System.currentTimeMillis()+".clip"));
		} else {
			...
			if (selectedFile != null) {
				...
				// getting the export manager associated to the provided format
				// index
				Export exportManager = exportHandlers[index];
				exportManager.export(relativeComponent, handle.getCanvas()
