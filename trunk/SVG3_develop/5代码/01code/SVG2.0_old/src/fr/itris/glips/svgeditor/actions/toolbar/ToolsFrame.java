 ...
 * 
 * the class allowing to display a frame containing the components used for
...
public class ToolsFrame {
	...
	/**
	 * the toggle button used to show or hide the frame, and that will be found
	 * in the tool bar
	 */
	...
	/**
	 * the runnable that should be executed when the visibility of the frame
	 * changes
	 */
	 ...
	 *            the id of a module
	 * @param label
	 *            the label that will be displayed in the title
	 * @param toolPanel
	 *            the panel that will be displayed
	 */
	public ToolsFrame(final Editor editor, String id, String label,
			final JPanel toolPanel) {
		...
		//modifyed by wangql
		if (toolPanel instanceof JWebBrowser) {
			
			internalFrame.getContentPane().add(toolPanel);
			toolPanel.setVisible(true);
			internalFrame.setVisible(true);
//			internalFrame.setBounds(190, 190, 400, 280);
			
			
//			NativeComponent.getNextInstanceOptions().setDestructionTime(
//					DestructionTime.ON_FINALIZATION);
			
			internalFrame.setSize(400, 300);
		} else {
			JScrollPane scrollpane = new JScrollPane(toolPanel);
			internalFrame.getContentPane().setLayout(
		...
		editor.getDesktop().addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent evt) {
				...
				Dimension parentSize = ToolsFrame.this.editor.getDesktop()
						.getSize();
...
//		 editor.getDesktop().setLayer(internalFrame, 0, 0);
		// getting the labels
		if (ResourcesManager.bundle != null) {

			try {
				showFrameLabel = ResourcesManager.bundle
						.getString("label_hidden_" + id.toLowerCase());
				hideFrameLabel = ResourcesManager.bundle
						.getString("label_shown_" + id.toLowerCase());
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		}
		...
		// handling the menuitem and the toggle button used to show or hide this
		// frame
		...
		final Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				Point location = SwingUtilities.convertPoint(frameButton
						.getParent(), frameButton.getLocation(), editor
						.getDesktop());
	...
	/**
	 * @param visibilityChangedRunnable
	 *            The visibilityChangedRunnable to set.
	 */
