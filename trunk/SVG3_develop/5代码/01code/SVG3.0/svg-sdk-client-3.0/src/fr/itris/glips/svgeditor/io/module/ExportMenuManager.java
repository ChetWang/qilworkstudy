...
public class ExportMenuManager {
	 ...
	 */
	public ExportMenuManager(IOModule iomod){
		this.iomodule = iomod;
		createMenuItems();
	}
		...
		for(int i=0; i<formats.length; i++){
			...
			listener=new ActionListener(){
				...
				public void actionPerformed(ActionEvent evt) {
					...
					SVGHandle handle=iomodule.getEditor().
						getHandlesManager().getCurrentHandle();
					
					iomodule.getEditor().getSelectionManager().setToRegularMode();
					iomodule.getEditor().getIOManager().getFileExportManager().export(
