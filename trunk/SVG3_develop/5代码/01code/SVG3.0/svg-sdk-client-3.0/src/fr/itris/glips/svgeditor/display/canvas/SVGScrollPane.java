...
import fr.itris.glips.svgeditor.display.canvas.rulers.Ruler;
import fr.itris.glips.svgeditor.display.canvas.rulers.RulersParametersManager;
import fr.itris.glips.svgeditor.display.handle.SVGHandle;
import fr.itris.glips.svgeditor.shape.AbstractShape;

/**
 * @author ITRIS, Jordi SUC the class of the scroll pane that will include the
 *         SVG canvas
 */
public class SVGScrollPane extends JPanel {
	 ...
	 * the corners
	 */
	protected Component northWestCornerBox, northEastCornerBox,
			southWestCornerBox;
	 ...
	 * 
	 * @param handle
	 *            the handle this scrollpane is linked to
	 */
	public SVGScrollPane(SVGHandle handle) {
		...
		// innerScrollPane.getViewport().setScrollMode(JViewport.SIMPLE_SCROLL_MODE);
	...
	public void initializeScrollpane() {
		...
		innerScrollPane.getViewport().setScrollMode(
		...
		Dimension cornerSize = new Dimension(innerScrollPane
		...
		horizontalRuler.setPreferredSize(new Dimension(innerScrollPane
				.getWidth(), cornerSize.height));
		...
		verticalRuler.setPreferredSize(new Dimension(cornerSize.width,
				innerScrollPane.getHeight()));
		...
		componentListener = new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent evt) {
				...
				if (listenersDisabled >= 0) {
					//
					canvas.setRenderedRectangle(new Rectangle(innerScrollPane
							.getViewport().getViewRect()), true, false);
		...
		innerScrollPane.getHorizontalScrollBar().addAdjustmentListener(
				adjustListener);
		innerScrollPane.getVerticalScrollBar().addAdjustmentListener(
				adjustListener);
			...
			public void mouseWheelMoved(MouseWheelEvent evt) {
				if (evt.isControlDown()) {
					if (evt.getWheelRotation() >= 0)
						canvas.getZoomManager().setToNextZoomInScale();
					else
						canvas.getZoomManager().setToNextZoomOutScale();
				} else {

					if (listenersDisabled >= 0
							&& evt.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {

						JScrollBar scrollBar = innerScrollPane
								.getVerticalScrollBar();
						scrollBar.setValue(scrollBar.getValue() + 2
								* scrollBar.getBlockIncrement()
								* evt.getWheelRotation());
						refreshRulers();
						canvas.setRenderedRectangle(new Rectangle(
								innerScrollPane.getViewport().getViewRect()),
								true, false);
					}
					else
					{
						...
						if(rotation > 0)
						{
							...
							.drawingAction(point,
									evt.getModifiers(),
									AbstractShape.DRAWING_MOUSE_WHEEL_DOWN);
						}
						else
						{
							//远离用户
							...
							.drawingAction(point,
									evt.getModifiers(),
									AbstractShape.DRAWING_MOUSE_WHEEL_UP);
						}
					}
				}
			}
	...
	public void dispose() {
		...
		innerScrollPane.getHorizontalScrollBar().removeAdjustmentListener(
				adjustListener);
		innerScrollPane.getVerticalScrollBar().removeAdjustmentListener(
				adjustListener);
	...
	public void updateRulers() {
		...
		RulersParametersManager manager = handle.getEditor()
	...
	public void setScrollValues(Dimension scrollValues) {
		...
		if (scrollValues != null) {
			...
			innerScrollPane.getHorizontalScrollBar().setValue(
					scrollValues.width);
			innerScrollPane.getVerticalScrollBar()
					.setValue(scrollValues.height);
		}
	}
	...
	public Dimension getScrollValues() {
		...
		return new Dimension(innerScrollPane.getHorizontalScrollBar()
				.getValue(), innerScrollPane.getVerticalScrollBar().getValue());
	}
	 ...
	 * 
	 * @param enabled
	 *            whether the listeners should be enabled or not
	 */
