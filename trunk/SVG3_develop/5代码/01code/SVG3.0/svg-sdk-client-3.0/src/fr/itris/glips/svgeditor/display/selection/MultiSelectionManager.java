...
import fr.itris.glips.svgeditor.shape.ManyShape;
...
public class MultiSelectionManager extends SelectionManager {
	...
	public void copyElement(Set<Element> elements, Point2D point) {
		...
		Rectangle2D rectangle = MultiAbstractShape.getElementsBounds(selection
				.getSVGHandle(), elements);
		double x = point.getX() - rectangle.getMinX();
		double y = point.getY() - rectangle.getMinY();
		ClipboardModule clipModule = (ClipboardModule) selection.getSVGHandle()
				.getEditor().getModule("Clipboard");
		clipModule.copy();
	...
	public void doTranslateAction(Set<Element> elements, Point2D firstPoint,
		...
		currentCanvasPainter = manyShape.showTranslateAction(selection
	...
	public void validateTranslateAction(Set<Element> elements,
		...
		// Shape sh = manyShape.getShape(selection.getSVGHandle(), element,
		// true);
		// add by yuxiang
		// 当拖动超出画板范围，则本次拖动无效
		Rectangle2D rect = manyShape.getBounds(selection.getSVGHandle(),
				elements);
		Point2D fPoint;
		if ((int) firstPoint.getX() == -1 && (int) firstPoint.getY() == -1) {
			...
			if (rect.getX() + xOffset < 0
					|| rect.getY() + yOffset < 0
					|| rect.getX() + rect.getWidth() + xOffset > currentSize
							.getX()
					|| rect.getY() + rect.getHeight() + yOffset > currentSize
							.getY()) {
		...
		if (undoRedoAction != null) {
			...
			selection.getSVGHandle().getUndoRedo().addActionList(actionlist,
					true);
		}
	}
	...
	public void validateAction(Set<Element> elements, Point2D firstPoint,
		...
		UndoRedoAction undoRedoAction = manyShape.validateAction(selection
				.getSVGHandle(), selection.getSelectionLevel(), elements, item,
