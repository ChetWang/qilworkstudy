...
public class ResourcesManager {
	 ...
	 *            the editor
	 */
	public ResourcesManager(EditorAdapter editor) {
	...
	public static String getPath(String resource) {
		...
		try {
			path = ResourcesManager.class.getResource(resource)
					.toExternalForm();
		} catch (Exception ex) {
			path = "";
		}
			...
			if (icons.containsKey(name)) {
				...
				if (iconsBundle != null) {
					...
					if (path != null && !path.equals("")) {
						...
						try {
							icon = new ImageIcon(new URL(getPath("icons/"
									+ path)));
						} catch (Exception ex) {
						}
	...
	public Document getResourceStore() {
		...
		if (resourceStore != null) {
			...
			try {
				resourceStore = factory
						.createDocument(getPath("xml/visualResourceStore.xml"));
			} catch (Exception ex) {
				resourceStore = null;
			}
		}
			...
			if (root != null) {
				...
				try {
					writer = new BufferedOutputStream(new FileOutputStream(
							new URI(getPath("xml/" + path)).getPath()));
				} catch (Exception ex) {
				}
	...
	public void notifyListeners() {
		...
		for (RecentFilesListener listener : new HashSet<RecentFilesListener>(
				recentFilesListeners)) {
