...
public class SelectionItem implements Comparable<SelectionItem> {
	...
	/**
	 * add by yux，2008.12.24 拓扑端点
	 */
	public static final int TERMINAL_POINT = 100;

	/**
	 * add by yux，2008.12.24 连接点
	 */
	public static final int CONNECTIVE_NODE_POINT = 101;
	...
	/**
	 * add by yux,2008.12.24 新世纪拓扑端点标记
	 */
	public static final int TERMINAL_POINT_STYLE = 100;
	/**
	 * add by yux,2008.12.24 新世纪连接点标记
	 */
	public static final int CONNECTIVE_NODE_STYLE = 101;

	/**
	 * predefined affine transforms
	 */
	private static AffineTransform rotate90 = AffineTransform
			.getRotateInstance(Math.PI / 2), rotateMinus90 = AffineTransform
			.getRotateInstance(-Math.PI / 2), rotate180 = AffineTransform
	...
	static {
		...
		// the point shape
		shapes
				.put(POINT_STYLE, new RoundRectangle2D.Double(-3, -3, 6, 6, 2,
						2));
		...
		shapes.put(TERMINAL_POINT_STYLE, new Ellipse2D.Double(-3, -3, 6, 6));

		Ellipse2D e1 = new Ellipse2D.Double(-5, -5, 10, 10);
		Ellipse2D e2 = new Ellipse2D.Double(-3, -3, 6, 6);

		GeneralPath gp = new GeneralPath(e1);
		gp.setWindingRule(GeneralPath.WIND_EVEN_ODD);
		gp.append(e2, false);
		shapes.put(CONNECTIVE_NODE_STYLE, gp);
	}

	/**
	 * the color used to paint the item when it is selected or not modi by
	 * yux,2008.12.24 新增terminalSelectionItemStrokeColor等四个terminal相关color
	 */
	protected final static Color singleSelectionItemStrokeColor = new Color(5,
			53, 174),
			singleSelectionItemFillColor = singleSelectionItemStrokeColor
					.brighter().brighter().brighter(),
			singleSelectionSelectedItemStrokeColor = new Color(255, 108, 0),
			singleSelectionSelectedItemFillColor = singleSelectionSelectedItemStrokeColor
					.brighter().brighter(),
			multiSelectionItemStrokeColor = new Color(78, 191, 23),
			multiSelectionItemFillColor = multiSelectionItemStrokeColor
					.brighter().brighter(),
			multiSelectionSelectedItemStrokeColor = new Color(205, 41, 141),
			multiSelectionSelectedItemFillColor = multiSelectionSelectedItemStrokeColor
					.brighter().brighter(),
			lockedSelectionItemStrokeColor = new Color(125, 125, 125),
			lockedSelectionItemFillColor = lockedSelectionItemStrokeColor
					.brighter().brighter(),
			lockedSelectionSelectedItemStrokeColor = new Color(175, 175, 175),
			lockedSelectionSelectedItemFillColor = lockedSelectionSelectedItemStrokeColor
					.brighter().brighter(),
			terminalSelectionItemStrokeColor = new Color(255, 255, 0),
			terminalSelectionItemFillColor = terminalSelectionItemStrokeColor
					.brighter().brighter(),
			terminalSelectionSelectedItemStrokeColor = new Color(255, 255, 0),
			terminalSelectionSelectedItemFillColor = terminalSelectionSelectedItemStrokeColor
					.brighter().brighter();
	...
	/**
	 * the index of the item, it is only useful when the type of the item is
	 * POINT or CONTROL_POINT and is used to get the position of the item in a
	 * path
	 */
	private int index = 0;
	 ...
	 */
	private Color itemStrokeColor, itemFillColor, selectedItemStrokeColor,
	 ...
	 *            the style that should be used to draw the item
	 * @param index
	 *            the index of the item, it is only useful when the type of the
	 *            item is POINT or CONTROL_POINT and is used to get the position
	 *            of the item in a path
	 * @param persistent
	 *            whether the selection item should be always painted when an
	 *            action occurs when it is selected
	 * @param relatedPoint
	 *            the related point, i.e. the point to which a line should be
	 *            drawn
	 */
	public SelectionItem(SVGHandle handle, Set<Element> elements,
			Point2D point, int type, int style, int index, boolean persistent,
			Point2D relatedPoint) {
		...
		if (type != LOCKED_POINT && type != TERMINAL_POINT
				&& type != CONNECTIVE_NODE_POINT) {

			itemStrokeColor = isMultiSelection ? multiSelectionItemStrokeColor
					: singleSelectionItemStrokeColor;
			itemFillColor = isMultiSelection ? multiSelectionItemFillColor
					: singleSelectionItemFillColor;
			selectedItemStrokeColor = isMultiSelection ? multiSelectionSelectedItemStrokeColor
					: singleSelectionSelectedItemStrokeColor;
			selectedItemFillColor = isMultiSelection ? multiSelectionSelectedItemFillColor
					: singleSelectionSelectedItemFillColor;

		} else if (type == TERMINAL_POINT || type == CONNECTIVE_NODE_POINT) {
			itemStrokeColor = terminalSelectionItemStrokeColor;
			itemFillColor = terminalSelectionItemFillColor;
			selectedItemStrokeColor = terminalSelectionSelectedItemStrokeColor;
			selectedItemFillColor = terminalSelectionSelectedItemFillColor;
		} else {
	...
	/**
	 * @return the index of the item, it is only useful when the type of the
	 *         item is POINT or CONTROL_POINT and is used to get the position of
	 *         the item in a path
	 */
	public int getIndex() {
		return index;
	...
	/**
	 * @return whether the selection item should be always painted when an
	 *         action occurs when it is selected
	 */
	...
	protected Cursor computeCursor() {
		...
		case NORTH:
			...
			cursorType = (style == ARROW_STYLE) ? Cursor.N_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;

		case SOUTH:

			cursorType = (style == ARROW_STYLE) ? Cursor.S_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;

		case EAST:

			cursorType = (style == ARROW_STYLE) ? Cursor.E_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;

		case WEST:

			cursorType = (style == ARROW_STYLE) ? Cursor.W_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;

		case NORTH_EAST:

			cursorType = (style == OBLIQUE_ARROW_STYLE) ? Cursor.NE_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;

		case SOUTH_EAST:

			cursorType = (style == OBLIQUE_ARROW_STYLE) ? Cursor.SE_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;

		case SOUTH_WEST:

			cursorType = (style == OBLIQUE_ARROW_STYLE) ? Cursor.SW_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;

		case NORTH_WEST:

			cursorType = (style == OBLIQUE_ARROW_STYLE) ? Cursor.NW_RESIZE_CURSOR
					: Cursor.HAND_CURSOR;
			break;
	...
	/**
	 * checks whether the given point is in the area of this item, if it is
	 * true, this item is filled with the color used to paint selected items
	 * 
	 * @param mousePoint
	...
	public boolean intersectsItemForMouseMove(Point mousePoint) {
		...
		final Color newStrokeColor = intersects ? selectedItemStrokeColor
				: itemStrokeColor;
		final Color newFillColor = intersects ? selectedItemFillColor
				: itemFillColor;
	...
	public void paint(Graphics2D g) {
		...
		boolean paint = (g.getClipBounds() != null && g.getClipBounds()
	...
	protected Shape computeShape() {
		...
		// concatenating the translate transform corresponding to the place of
		// the
		// item on the canvas to the founds affine transform
		af.preConcatenate(AffineTransform.getTranslateInstance(point.getX(),
				point.getY()));
	...
	protected AffineTransform getTransformForShape() {
		...
		case LOSANGE_STYLE:
		case TERMINAL_POINT_STYLE:
		case CONNECTIVE_NODE_STYLE:
