...
import com.nci.svg.sdk.other.NciTargetMath;
import com.nci.svg.sdk.other.SVGNciPropertyItem;
...
/**
 * the class allowing to change the properties of a node by modifying its
...
public class SVGProperties extends ModuleAdapter {
    ...
    public void openProperties() {
        propertiesFrame.openProperties();
        return;
    }
     ...
     *            the editor
     */
    public SVGProperties(EditorAdapter editor) {
    ...
    private void initModule() {
        ...
        final HandlesListener svgHandleListener = new HandlesListener() {
            ...
            public void handleChanged(SVGHandle currentHandle,
                ...
                final SVGHandle handle = getEditor().getHandlesManager()
                        .getCurrentHandle();

                if (handle != null && handle.getSelection() != null) {

                    manageSelection(new HashSet<Element>(handle.getSelection()
                            .getSelectedElements()));
                    ...
                    handle.getSelection().addSelectionChangedListener(
                            selectionListener);
             ...
             *            the set of the selected elements
             */
        ...
        propertiesFrame = new ToolsFrame(editor, idproperties, labelproperties,
                propertiesPanel);
...
//        menuItems.put("ToolFrame_" + this.idproperties, propertiesFrame
//                .getMenuItem());
//        return menuItems;
//    }
...
//    }
    ...
    protected void handleProperties(LinkedList list) {
        ...
        if (handle != null && list != null && list.size() > 0) {
            ...
            if (treeProperties != null) {
                ...
                // the map associating a tab to its label and the map
                // associating a tab to a list of property item objects
                ...
                if (treeProperties != null) {
                    ...
                    for (Node cur = treeProperties.getFirstChild(); cur != null; cur = cur
                            .getNextSibling()) {
                        ...
                        if (cur.getNodeName().equals("tab")) {
                            ...
                            //add by yuxiang
                            //获取个性化属性
                            type = ((Element) cur).getAttribute("type");
                            if (type != null && type.equals("solidified")
                                    && snodes.size() == 1) {
                                tabMap.put(name, "个性化");

                                // gets the property items list and puts it into
                                // the map
                                propertyItemsList = getNciPropertyList(snodes);
                                propertyItemsMap.put(name, propertyItemsList);
                                //end add
                            } else if (name != null && !name.equals("")) {

                                // gets the label of the tab and puts it into
                                // the map
                                if(name.equals("metadata"))
                                {
                                    tabMap.put(name, "metadata");
                                    
                                    propertyItemsList = getMetadataPropertyList(snodes);
                                    propertyItemsMap.put(name, propertyItemsList);
                                }
                                else
                                {
                    ...
                    // creates the panel
                    if (tabMap != null && propertyItemsMap != null
                            && tabMap.size() > 0 && propertyItemsMap.size() > 0) {
                        ...
                        widgetsPanel = new SVGPropertiesWidgetsPanel(this,
                                tabMap, propertyItemsMap);
                    }

                    // adds the property panel into the container and displays
                    // it
    ...
    public Document normalizeXMLProperties(Document doc) {
        ...
        // each tab node is appended to the node which have declared having a
        // child whose type is like one of the predefined tabs
        if (doc != null) {
            ...
            if (root != null) {
                ...
                // the map associating the name attributes of a tab to its tab
                // node
                ...
                // finds the node that contains the predefined node tabs, and
                // adds all its children to the tab map
                ...
                for (cur = root.getFirstChild(); cur != null; cur = cur
                    ...
                    if (cur.getNodeName().equals("define")) {
                        ...
                        for (tab = cur.getFirstChild(); tab != null; tab = tab
                                .getNextSibling()) {
                ...
                // appends the tab nodes to the node that define a link to a
                // predefined tab
                for (cur = root.getFirstChild(); cur != null; cur = cur
                        .getNextSibling()) {
                    ...
                    if (cur.getNodeName().equals("module")) {
                        ...
                        // gets the "use" nodes defined in the "module" node and
                        // the "tab" nodes that will be used to replace them
                        for (use = cur.getFirstChild(); use != null; use = use
                                .getNextSibling()) {
                        ...
                        for (tab = cur.getFirstChild(); tab != null; tab = cur
                                .getFirstChild()) {
    ...
    /**
     * gets the subtree describing the properties that can be changed for the
     * given list of nodes
     * 
     * @param list
     *            a list of nodes
            ...
            while (it.hasNext()) {
                ...
                if (current != null) {
                    ...
                    if (name != null && !name.equals(current.getNodeName())) {
                        ...
                        subTree = intersectXMLProperties(subTree,
                                getXMLProperties(current));
                    }
    ...
    /**
     * gets a subtree that contains the nodes that can be found in the node1
     * subtree AND in the node2 subtree
     * 
     * @param node1
     *            a subtree
     * @param node2
     *            a subtree
     * @return a subtree that contains the nodes that can be found in the node1
     *         subtree AND in the node2 subtree
     */
    protected Node intersectXMLProperties(Node node1, Node node2) {
        ...
        if (node1 != null && node2 != null) {
            ...
            Node tab = null, ctab = null, tab2 = null, property = null, cproperty = null, property2 = null;
            String name = null;

            for (tab = node1.getFirstChild(); tab != null; tab = tab
                    .getNextSibling()) {
                ...
                if (tab.getNodeName().equals("tab")) {
                    ...
                    if ((tab2 = getTab(node2, tab)) != null) {
                        ...
                        // for each property in the current tab in node1, tests
                        // if it is in node2
                        for (property = tab.getFirstChild(); property != null; property = property
                                .getNextSibling()) {
     ...
     *            a subtree
     * @param tab
     *            a node whose name is "tab"
     * @return the tab node whose attribute name is equal to the attribute name
     *         of the given tab or null if the node does not contain the tab
     */
    protected Node getTab(Node node, Node tab) {
        ...
        if (node != null && tab != null && tab.getNodeName().equals("tab")) {
            ...
            if (tabName != null && !tabName.equals("")) {
                ...
                // for each tab in node2, test if it has the same name as the
                // given tab
                for (tab2 = node.getFirstChild(); tab2 != null; tab2 = tab2
                        .getNextSibling()) {
    ...
    /**
     * @param tab
     *            the tab in which the property is contained
     * @param property
     *            the property node
     * @return the property node whose attribute name is equal to the attribute
     *         name of the given property or null if the node does not contain
     *         the property
     */
    protected Node getProperty(Node tab, Node property) {
        ...
        if (tab != null && property != null && property instanceof Element) {
            ...
            if (propertyName != null && !propertyName.equals("")
                    && propertyType != null && !propertyType.equals("")) {

                for (prop = tab.getFirstChild(); prop != null; prop = prop
                        .getNextSibling()) {
                    ...
                    if (prop.getNodeName().equals("property")) {
                        ...
                        // tests if the current property name and type are equal
                        // to the gievn property name and type
                        if (propertyName2 != null && propertyType2 != null
                                && propertyName2.equals(propertyName)
                                && propertyType2.equals(propertyType)) {
        ...
        if (docProperties != null && node != null) {
            ...
            if (name != null && !name.equals("")) {
                ...
                if (root != null) {
                    ...
                    // for each root child, searches the one whom value of the
                    // "name" attribute is the type of the given node
                    for (current = root.getFirstChild(); current != null; current = current
                            .getNextSibling()) {
    ...
    protected LinkedList getPropertyList(LinkedList nodelist, Node subTree) {
        ...
        if (subTree != null) {
            ...
            for (cur = subTree.getFirstChild(); cur != null; cur = cur
                    .getNextSibling()) {
    ...
    protected SVGPropertyItem getProperty(LinkedList nodelist, Node property) {
        ...
        if (property != null) {
            ...
            if (property.hasChildNodes()) {
                ...
                for (cur = property.getFirstChild(); cur != null; cur = cur
                        .getNextSibling()) {
                    ...
                    if (cur.getNodeName().equals("item")) {
                        ...
                        if (itemName != null && !itemName.equals("")
                                && itemValue != null && !itemValue.equals("")) {
            ...
            if (type != null && name != null && valueType != null
                    && !type.equals("") && !name.equals("")
                    && !valueType.equals("")) {

                propertyItem = new SVGPropertyItem(this, nodelist, type, name,
                        valueType, defaultValue, constraint, values);
            }
        }
    ...
    /**
     * 读取第三方配置文件，生成metadata数据域
     * @param nodelist:需要修改的内容
     * @return:
     */
    protected LinkedList getMetadataPropertyList(LinkedList nodelist) {
        return null;
    }
    
    //add by yuxiang
    //获取个性化属性
    protected LinkedList getNciPropertyList(LinkedList nodelist) {
        Element gElement = (Element) nodelist.iterator().next();
        try {
           
            Element element = (Element) Utilities.findNode(
                    "*[@id='adjustable']", gElement);
            LinkedList list = new LinkedList();
            if (gElement != null) {

                Node cur = null;
                String type = "";
                NciTargetMath math = new NciTargetMath(editor);
                
                SVGPropertyItem item = null;
                item = getgProperty(gElement, "nci-targettype", "指标类型");
                if (item != null)
                    list.add(item);
                item = getgProperty(gElement, "nci-targeturl", "连接");
                if (item != null)
                    list.add(item);
                item = getgComboProperty(gElement, "nci-targetorder", "区域划分",math);
                if (item != null)
                    list.add(item);
                // for each property node
                for (cur = gElement.getFirstChild(); cur != null; cur = cur
                        .getNextSibling()) {
                    if (cur instanceof Element) {
                        type = ((Element) cur).getAttribute("nci_type");
                        if (type != null && type.equals("adjustable")) {

                            item = getCustomProperty(cur, math);
                            if (item != null)
                                list.add(item);
                        } else if (type != null && type.equals("target")) {
                            math.setTarget((Element) cur);
                        } else if (type != null
                                && type.equals("nci_target_pointer")) {
                            math.setTarget_center((Element) cur);
                        }
    ...
    protected SVGPropertyItem getgProperty(Node property, String strName,
            String showText) {

        SVGNciPropertyItem propertyItem = null;

        if (property != null) {

            String type = "attribute", name = "property_", valueType = "singleentry", defaultValue = "", constraint = "normal";
            name += strName;
            //the attributes of the property node
            ...
            if (type != null && name != null && valueType != null
                ...
                propertyItem = new SVGNciPropertyItem(this, property, type,
                        name, valueType, defaultValue, constraint, showText,null);
            }
        }
    ...
    protected SVGPropertyItem getgComboProperty(Node property, String strName,
            String showText,NciTargetMath math) {

        SVGNciPropertyItem propertyItem = null;

        if (property != null) {

            String type = "attribute", name = "property_", valueType = "combo", defaultValue = "顺时针", constraint = "normal";
            name += strName;
            //the attributes of the property node
            ...
            if (type != null && name != null && valueType != null
                ...
                propertyItem = new SVGNciPropertyItem(this, property, type,
                        name, valueType, defaultValue, constraint, showText,values);
                propertyItem.setMath(math);
                math.setMap(showText, propertyItem);
            }
        }
    ...
    protected SVGPropertyItem getCustomProperty(Node property,
            NciTargetMath math) {
        ...
        if (property != null) {
            ...
            String type = "child", name = "property_#text", valueType = "singleentry", defaultValue = "", constraint = "normal";

            //the attributes of the property node
            String showText = ((Element) property).getAttribute("showtext");
    ...
    protected boolean handleDesc()
    {
        ...
        if(handle == null)
            return false;
        Element desc = (Element)handle.getCanvas().getDocument().getDocumentElement().getElementsByTagName("desc").item(0);
        LinkedList list = new LinkedList();
        list.add(desc);
        if (handle != null && list != null && list.size() > 0) {
	 ...
	 */
	public Document getDocProperties() {
