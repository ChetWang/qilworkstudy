...
public class PNGExport extends Export{
	...
	protected PNGExport(FileExport fileExport) {
		...
		if(fileExport.getEditor().getParent() instanceof Frame){
			...
			exportDialog=new PNGExportDialog((Frame)fileExport.getEditor().getParent(),fileExport.getEditor());
			
		}else{
			
			exportDialog=new PNGExportDialog((JDialog)fileExport.getEditor().getParent(),fileExport.getEditor());
		}
	}
	...
	public void export(JComponent relativeComponent, 
		...
		monitor=new ExportMonitor(
				fileExport.getEditor().getParent(), 0, 100, FileExport.prefixLabels[1]);
