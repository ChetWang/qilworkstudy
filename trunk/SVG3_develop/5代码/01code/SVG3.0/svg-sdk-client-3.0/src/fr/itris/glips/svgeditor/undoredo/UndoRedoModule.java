...
public class UndoRedoModule extends ModuleAdapter{
	 ...
	 */
	public UndoRedoModule(EditorAdapter editor){
		super(editor);
		initUndoRedo();
	}
					...
					public synchronized void actionPerformed(ActionEvent e) {
						...
						if(e.getModifiers()==InputEvent.BUTTON1_DOWN_MASK || 
								e.getModifiers()==InputEvent.BUTTON1_MASK || 
									getEditor().getHandlesManager().keyStrokeActsOnSVGFrame()){
							
							//getting the current svg handle
							SVGHandle handle=getEditor().getHandlesManager().getCurrentHandle();
	...
	public Collection<PopupItem> getPopupItems() {
		...
		PopupItem undoItem=new PopupItem(getEditor(), idundo, labelundo, "Undo"){
		...
		PopupItem redoItem=new PopupItem(getEditor(), idredo, labelredo, "Redo"){
