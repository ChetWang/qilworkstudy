...
public class ResourceImageManager{
    ...
    /**
     * the map associating a svg handle to a map associating the id of a resource to the
     * imageRepresentation object representing this resource
     */
    private final Map<SVGHandle, Map<String, ImageRepresentation>> handleToIdToImages=
    	Collections.synchronizedMap(new HashMap<SVGHandle, Map<String, ImageRepresentation>>());
     ...
     */
    public ResourceImageManager(EditorAdapter editor){
        ...
        editor.getHandlesManager().addHandlesListener(new HandlesListener(){
        	...
        	public void handleChanged(SVGHandle currentHandle, Set<SVGHandle> handles) {
	            ...
	            //removes the image representations that belongs to disposed svg handles
    ...
    public ResourceRepresentation getResourceRepresentation(
    	SVGHandle svgHandle, String resourceId, boolean useSmallImage, Component componentToRefresh){
     ...
     */
    protected EditorAdapter getSVGEditor() {
        return editor;
    }
            ...
            if(handleToIdToImages.containsKey(svgHandle)){
                ...
                if(idToImageMap!=null){
					...
					Map<String, Element> resources=
						svgHandle.getSvgResourcesManager().getResourcesFromDefs(	
    ...
    protected void createNewImage(SVGHandle svgHandle, String resourceId){
        ...
        if(svgHandle!=null && resourceId!=null && ! resourceId.equals("")){
            ...
            try{
                resourceElement=svgHandle.getScrollPane().getSVGCanvas().getDocument().getElementById(resourceId);
            }catch (Exception ex){}
            ...
            if(resourceElement!=null){
                ...
                if(	resourceElement.getNodeName().equals("linearGradient") || 
                    ...
                    if(clonedResourceElement!=null){
                        ...
                        try{
                            initialWidth=EditorToolkit.getPixelledNumber(initialRoot.getAttributeNS(null, "width"));
                            initialHeight=EditorToolkit.getPixelledNumber(initialRoot.getAttributeNS(null, "height"));
                        }catch (Exception ex){}
                        ...
                        if(resourceElement.getNodeName().equals("linearGradient")){
                            ...
                            if(resourceElement.getAttributeNS(null, "patternUnits").equals("userSpaceOnUse")){
                                ...
                                //normalizing the values for the vector to fit the rectangle
                                try{
                                    x=Double.parseDouble(resourceElement.getAttributeNS(null, "x"));
                                    y=Double.parseDouble(resourceElement.getAttributeNS(null, "y"));
                                    w=Double.parseDouble(resourceElement.getAttributeNS(null, "w"));
                                    h=Double.parseDouble(resourceElement.getAttributeNS(null, "h"));
     ...
     */
    protected ResourceRepresentation getRepresentation(
    	SVGHandle svgHandle, String resourceId, boolean useSmallImage, 
    ...
    public class ResourceRepresentation extends JPanel{
    	 ...
    	 * @param componentToRefresh the component to refresh
    	 */
    	protected ResourceRepresentation(SVGHandle svgHandle, 
    			String resourceId, boolean useSmallImage, Component componentToRefresh){
