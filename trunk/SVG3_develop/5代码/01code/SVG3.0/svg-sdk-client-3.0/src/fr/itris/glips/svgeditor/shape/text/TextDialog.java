...
public class TextDialog extends TitledDialog {
	 ...
	 */
	public TextDialog(TextShape textShape, Frame parent, EditorAdapter editor) {
	 ...
	 *            the parent of the dialog
	 */
	public TextDialog(TextShape textShape, JDialog parent, EditorAdapter editor) {
	 ...
	 *            the parent of the dialog
	 */
	public TextDialog(ModuleAdapter module, JDialog parent, EditorAdapter editor) {
	 ...
	 *            the parent of the dialog
	 */
	public TextDialog(ModuleAdapter module, Frame parent, EditorAdapter editor) {
	...
	protected JPanel buildContentPanel() {
		...
		final String titleDialogMessageLabel = bundle
				.getString("TextShapeDialogMessage");
		String textShapeTextPromptLabel = bundle
				.getString("TextShapeTextPrompt");
		final String textShapeErrorEmptyStringLabel = bundle
				.getString("TextShapeErrorEmptyString");
		...
		textFieldListener = new CaretListener() {
			...
			public void caretUpdate(CaretEvent e) {
				...
				if (textField.getText().equals("")) {
					...
					setMessage(textShapeErrorEmptyStringLabel,
							TitledDialog.ERROR_TYPE);
					okButton.setEnabled(false);

				} else {

					setMessage(titleDialogMessageLabel,
							TitledDialog.INFORMATION_TYPE);
		...
		ActionListener buttonsListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (evt.getSource().equals(okButton)) {
					...
					if (textShape != null)
						textShape.createElement(currentHandle, textField
								.getText());
					else {
						Map map = new HashMap();
						map.put("text", textField.getText());
						module.handleOper("0", map);
					}
				}

				setVisible(false);
				editor.getHandlesManager().getCurrentHandle().getSelection()
						.setBTextDialog(false);
			}
		};
	...
	public void showDialog(JComponent relativeComponent, SVGHandle handle) {
		...
		showDialog(relativeComponent, handle, "");
	}
