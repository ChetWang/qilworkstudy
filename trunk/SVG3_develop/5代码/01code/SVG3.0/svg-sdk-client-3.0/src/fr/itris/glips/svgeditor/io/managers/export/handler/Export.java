...
public abstract class Export {
	...
	protected Export(FileExport fileExport) {
		...
		errorMessage = ResourcesManager.bundle
				.getString("FileExportErrorMessage");
	}
	 ...
	 * @param fileExport
	 *            the file export to which the dialogs will be shown
	...
	/**
	 * exports the svg document into the file denoted by the given path with the
	 ...
	 * @param imageFile
	 */
	protected void writeToClipBoard(File imageFile) {
		ImageTransferable clipContent = new ImageTransferable(imageFile);
	 ...
	 * @param destFile
	 *            the destination export file
	 * @param useAlpaChannel
	 *            whether the image should use a alpha channel
	 */
	protected void createImage(Document document, final File destFile,
			boolean useAlpaChannel) {
		...
		SVGDocumentImageCreator imageCreator = new SVGDocumentImageCreator(
				document, monitor,fileExport.getEditor());
	 ...
	 * 
	 * @param image
	 *            the image corresponding to the svg document
	 * @param destFile
	 *            the destination export file
	 */
