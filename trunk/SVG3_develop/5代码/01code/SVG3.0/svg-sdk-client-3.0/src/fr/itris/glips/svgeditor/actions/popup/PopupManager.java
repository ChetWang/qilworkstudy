...
public class PopupManager {
	...
	/**
	 * the map (the description of the popup menu) associating the name of a
	 * group of popup items to the list of the popup items names
	 */
	private final Map<String, LinkedList<String>> popupItemsDescription = new LinkedHashMap<String, LinkedList<String>>();
	 ...
	 *            the editor
	 */
	public PopupManager(EditorAdapter editor) {
	...
	protected void retrieveModulePopupItems() {
		...
		Collection<ModuleAdapter> modules = getSVGEditor().getSVGModuleLoader()
				.getModules();
		Collection<PopupItem> itemsList = null;

		// for each module, gets the popup items linked with the given list of
		// nodes
	 ...
	 *            a handle
	 * @param location
	 *            the location of the mouse click wher the popup should appear
	 */
	public void showPopup(SVGHandle handle, Point location) {

		if (editor.getModeManager().isShowPopupMenu()) {
			...
			if (handle != null && location != null && popupMenu != null) {
				...
				if (popupItems != null && popupItems.size() > 0) {
					...
					// for each group of menu items, fills the pop up menu and
					// put
					// popup
					// separators between each group of them
					for (LinkedList<String> itemNames : popupItemsDescription
							.values()) {
						...
						if (itemNames != null) {
							...
							for (String itemName : itemNames) {
								...
								// if the name of the menuitem of the popup
								// model is
								// contained
								// in the list of the names of the menuitems
								// that
								// should be displayed
								if (itemName != null
										&& popupItems.containsKey(itemName)) {
							...
							if (isGroupEnabled) {
								...
								for (JMenuItem mi : popupItemsToAdd) {
									...
									if (mi != null) {
										...
										// the menu item is added to the popup
										// menu
										popupMenu.add(mi);
									}
								}
								...
								if (hasAddedAnItem
										&& i < popupItemsDescription.size() - 2) { // wangql
									// changed
									// -1
									// to
									// -2,多了一个seperator

									// if other groups of menu items could be
									// added
									// and if menu items
									// from the previous group have been added,
									// a
									// new separator is added
									// the "if" sentence is added by wangql
									// if (((NCIViewEditModule) editor
									// .getModule(NCIViewEditModule.
									// NCI_View_Edit_ModuleID))
									// .getViewEdit_mode().equals(
									// NCIViewEditModule.EDIT_MODE))
					...
					try {
						((ViewEditModuleAdapter) editor
								.getModule(ViewEditModuleAdapter.NCI_View_Edit_ModuleID))
								.handleSeparators(popupMenu.getComponents());
					} catch (Exception e) {

					}

					popupMenu.show(handle.getScrollPane().getSVGCanvas(),
	...
	/**
	 * parses the document to get the menu items specified in the "popup.xml"
	 * file
	 */
	protected void parseXMLPopupMenuItems() {
		...
		if (doc != null) {
			...
			if (root != null) {
				...
				for (Node current = root.getFirstChild(); current != null; current = current
						.getNextSibling()) {

					if (current instanceof Element
							&& current.getNodeName().equals("group")) {
						...
						if (groupName != null && !groupName.equals("")) {
							...
							for (item = current.getFirstChild(); item != null; item = item
									.getNextSibling()) {
	...
	public Map<String, PopupItem> getPopupItems() {
		return popupItems;
	}
