...
public class HandlesManager {
	 ...
	 */
	private List<SVGHandle> handles = new CopyOnWriteArrayList<SVGHandle>();
	 ...
	 */
	private Set<HandlesListener> handlesListeners = new CopyOnWriteArraySet<HandlesListener>();
	 ...
	 *            the editor
	 */
	public HandlesManager(EditorAdapter editor) {
	...
	/**
	 * @return whether the key strokes act on an open and selected svg frame
	 */
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				if (currentHandle != null) {
					...
					currentHandle.getSVGFrame().getFrameMenuItem().setSelected(
							true);
				}
			}
		};
	...
	public void handleCreated() {
		for (HandlesListener listener : handlesListeners) {
			...
			if (listener != null) {
				...
				listener.handleCreated(currentHandle);
			}
		}
	}
	 ...
	 * 
	 * @param handle
	 *            the svg handle whose menuitem will be inserted into the menu
	 *            bar
	 */
	protected void addHandleInMenu(SVGHandle handle) {
		...
		handle.getSVGFrame().getFrameMenuItem().addActionListener(
				new ActionListener() {
		...
		if (menubar != null) {
			...
			menubar.addUnknownMenuItem(idmenuframe, handle.getSVGFrame()
					.getFrameMenuItem());
	 ...
	 * 
	 * @param handle
	 *            the svg handle whose menuitem will be removed from the menu
	 *            bar
	 */
	protected void removeHandleInMenu(SVGHandle handle) {
		...
		if (handle != null) {
			...
			menubar.removeUnknownMenuItem(idmenuframe, handle.getSVGFrame()
					.getFrameMenuItem());
	 ...
	 * 
	 * @param name
	 *            the name of the new svg handle
	...
	public SVGHandle createSVGHandle(String name) {
		...
		// }
		//
		// SVGHandle handle = new SVGHandle(name, editor);
		//
		...
		return createSVGHandle(name, SVGHandle.HANDLE_TYPE_SVG);
	}

	/**
	 * 创建svghandle对象，added by wangql.
	 * 
	 * @param name
	 * @param handleType
	 *            指定是何种handle类型
	 * @return
	 */
	public SVGHandle createSVGHandle(String name, int handleType) {
		...
		if (name != null && !name.equals("")) {
			...
			SVGHandle handle = new SVGHandle(name, handleType, editor);
	 ...
	 * 
	 * @param oldName
	 *            the current name of the handle
	 * @param newName
	...
	public void changeName(String oldName, String newName) {
		...
		if (newName != null && !newName.equals("")) {
			...
			ClipboardModule module = (ClipboardModule) editor
					.getModule("Clipboard");
			module.resetDefsHis(oldName, newName);
	...
	public void setCurrentHandle(String name) {
		...
		if (handle != null) {
			...
			handles.add(handles.size(), handle);
			try {
				// if(handle.getSVGFrame().isIcon())
				{
					// handle.getSVGFrame().setMaximum(false);
					handle.getSVGFrame().setSelected(true);
					// handle.getSVGFrame().setIcon(true);
					// handle.getSVGFrame().setMaximum(true);
				}
	...
	/**
	 * 设置当前的图元管理界面. added by wangql
	 * 
	 * @param handle
	 */
	public void setCurrentUnitHandle(SVGHandle handle) {
	...
	/**
	 * @return the handle whose frame containing the svg canvas is currently
	 *         selected
	 */
	...
	/**
	 * removes the handle from the list : the frame containing the svg canvas
	 * will be hidden
	 * 
	 * @param name
	 *            the name of the svg handle to remove
	 */
	public void removeHandle(String name) {
		...
		if (handle != null) {
			...
			// if it is not the last handle in the list, another handle is taken
			// into account
			if (handles.size() > 0) {
	...
	public void removeHandleWithoutDisposingGUnit(String name) {
		SVGHandle handle = getHandle(name);
		...
		if (handle != null) {
			...
			// if it is not the last svg handle in the list, another handle is
			// taken into account
			// if (handles.size() > 0) {
			//
			// setCurrentHandle(handles.get(handles.size() - 1).getName());
	...
	public void addHandleAndSetToCurrent(SVGHandle handle) {
		handles.add(handles.size(), handle);
		currentHandle = handle;
	}
	...
	public static void main(String[] xxx) {
		Color o = Constants.THUMBNAIL_PERSONAL_BACKGROUND;
		System.out.println(0x00FFFFFF - (~o.getRGB()));
		o = Constants.THUMBNAIL_RELEASED_BACKGROUND;
		System.out.println(0x00FFFFFF - (~o.getRGB()));
		System.out.println(~0xe999eb);
	}
}
