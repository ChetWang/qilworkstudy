...
public class BMPExport extends Export {
	...
	protected BMPExport(FileExport fileExport) {
		...
		// creating the dialog
		if (getFileExport().getEditor().getParent() instanceof Frame) {

			exportDialog = new BMPExportDialog((Frame) fileExport.getEditor()
					.getParent(), fileExport.getEditor());

		} else {

			exportDialog = new BMPExportDialog((JDialog) fileExport.getEditor()
					.getParent(), fileExport.getEditor());
		}
	}
	...
	public void export(JComponent relativeComponent, Document document,
		...
		monitor = new ExportMonitor(getFileExport().getEditor().getParent(), 0,
				100, FileExport.prefixLabels[2]);
				...
				try {
					...
					writer
							.addIIOWriteProgressListener(new IIOWriteProgressListener() {
								...
								public void imageProgress(ImageWriter wr,
										float value) {
								...
								public void imageStarted(ImageWriter wr,
										int arg1) {
								}
								...
								public void thumbnailProgress(ImageWriter wr,
										float arg1) {
								}

								public void thumbnailStarted(ImageWriter wr,
										int arg1, int arg2) {
								}
							});
