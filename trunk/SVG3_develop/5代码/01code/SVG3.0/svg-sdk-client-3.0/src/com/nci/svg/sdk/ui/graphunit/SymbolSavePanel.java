 ...
 * @author Qil.Wong
 */
public class SymbolSavePanel extends EditorPanel {
	...
	private GraphUnitModuleAdapter graphunitModule = null;
	...
	public SymbolSavePanel(GraphUnitModuleAdapter graphunitModule) {
		super(graphunitModule.getEditor());
		this.graphunitModule = graphunitModule;
		initComponents();
	 ...
	 * 初始化最最初始的数据
	 * 
	 * @param graphunit_or_template
	 *            区别是图元还是模板
	 * @see fr.itris.glips.svgeditor.display.handle.SVGHandle#HANDLE_TYPE_SYMBOL_GRAPH_UNIT_NORMAL
	 *      表示图元
	 * @see fr.itris.glips.svgeditor.display.handle.SVGHandle#HANDLE_TYPE_SYMBOL_TEMPLATE
	 *      表示模板
	 */
	public void initData(int graphunit_or_template) {
		dataIniting = true;
		((DefaultComboBoxModel) typeCombo.getModel()).removeAllElements();
		((DefaultComboBoxModel) typeCombo.getModel()).addElement(select);
		// 初始化类型combobox
		List<SymbolTypeBean> types = null;
		if (graphunit_or_template == SVGHandle.HANDLE_TYPE_SYMBOL_GRAPH_UNIT_NORMAL) {
			types = editor.getSymbolManager().getSymbolTypeNames(NCIEquipSymbolBean.SYMBOL_TYPE_GRAPHUNIT);
		} else if (graphunit_or_template == SVGHandle.HANDLE_TYPE_SYMBOL_TEMPLATE) {
			types = editor.getSymbolManager().getSymbolTypeNames(NCIEquipSymbolBean.SYMBOL_TYPE_TEMPLATE);
		}
		for (SymbolTypeBean typeBean : types) {
			((DefaultComboBoxModel) typeCombo.getModel()).addElement(typeBean);
//			((DefaultComboBoxModel) typeCombo.getModel()).addElement(typeBean.getSymbolSubCode());
		}
	...
	public void setValue(NCIEquipSymbolBean bean) {
		...
		//TODO 设置图元、模板保存时的值 deprecated by wangql @2008.12.11-13:41
//		try {
//			this.nameText.setText(bean.getGraphUnitName());
//			this.typeCombo.setSelectedItem(bean.getVariety());
...
//					.getSelectedItem()).getSymbolBean().getGraphUnitGroup());
		...
		bean.setName(nameText.getText().trim());
//		bean.setGraphUnitStatus(statusCombo.getSelectedItem().toString());
		bean.setVariety((SimpleCodeBean)typeCombo.getSelectedItem());
	}
	...
	public boolean checkInput(NCIEquipSymbolBean checkedSymbol) {
		...
		if (statusCombo.getSelectedIndex() > 0) {
			...
			if (relatedCombo.getSelectedIndex() > 0) {
				...
				// TODO modified by wangql, 相同状态的图元已经存在还需判断
//				String group = relatedSymbol.getGraphUnitGroup();
				...
				// String existSymbolID =
				// statusBean.getStatusSymbol(editor.getGraphUnitManager().getSymbolsStatus(),
				// status);
	...
	private void showRelatedGraphUnits() {
		if (!dataIniting) {
			...
			if (!statusCombo.getSelectedItem().equals(
				...
				//FIXME 这里有误，这个thumbnailmap只是显示过的thumbnails，如果选择了还没有选择过的类型，则结果为空
//				LinkedHashMap<String, NCIThumbnailPanel> thumbnailsMap = editor
//						.getThumnailsMap().get(type);
				LinkedHashMap<String, NCIThumbnailPanel> thumbnailsMap = null;
//				HashMap<String,>
