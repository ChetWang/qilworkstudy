...
import com.nci.ums.subscribe.SubscriptionService;
import com.nci.ums.tactics.ChannelPolicy;
...
public class UMSCommandCenter implements CommandCenter {
	...
	public UMSCommandCenter() {
		...
		this.setMsgInsertSQL(DBUtil_V3
				.createOutMsgInsertSQL(DBUtil_V3.MESSAGE_UMS_SEND_READY));
	...
	public String start(BasicMsg[] basicMsgs) throws SQLException,
		...
		// 将包含多个receiver的basicMsg拆分成一个receiver对应一个BasicMsg
		basicMsgs = parseSingeReceiver(basicMsgs);
		UMSMsg[] msgs = new UMSMsg[basicMsgs.length];

		boolean firstAck = true;
		for (int i = 0; i < basicMsgs.length; i++) {
			// 订阅消息组成
			if (basicMsgs[i].getUmsFlag() == BasicMsg.BASICMSG_TOPIC_UMS) {
				SubscriptionService.getInstance()
						.implSubscription(basicMsgs[i]);
			}
			// 消息策略处理
	 ...
	 * @return
	 */
	private BasicMsg[] parseSingeReceiver(BasicMsg[] basicMsgs) {
		ArrayList list = new ArrayList();
		for (int i = 0; i < basicMsgs.length; i++) {
			Participant[] receivers = basicMsgs[i].getReceivers();
			if (receivers != null) {
				// MODIFY BY zhouhm 2009-11-08
				// 当接受者为空时按照系统消息发送给消息订阅者
				// int size = receivers.length;
				// if(size<=0){
				// SubscriptionService.getInstance().implSubscription(basicMsgs[i]);
				// }
				// receivers = basicMsgs[i].getReceivers();
				// if(receivers == null){
				// continue;
				// }
				// END MODIFY
				for (int n = 0; n < receivers.length; n++) {
					BasicMsg clonedBasicMsg = basicMsgs[i].deepClone();
					clonedBasicMsg
							.setReceivers(new Participant[] { receivers[n] });
					list.add(clonedBasicMsg);
				}
			}
		}
		int size = list.size();
		...
		BasicMsg[] newBasicMsgs = new BasicMsg[size];
		for (int i = 0; i < list.size(); i++) {
			newBasicMsgs[i] = (BasicMsg) list.get(i);
		}
		return newBasicMsgs;
	}
	...
	private void storeSendMsg(Connection conn, PreparedStatement msgPrep,
		...
		dbUtil.executeOutMsgInsertStatement(msgPrep, attachPrep, msg,
				DBUtil_V3.MESSAGE_UMS_SEND_READY, conn);
	}

	public String transmit(UMSMsg[] msgs) throws OutOfMaxSequenceException {
		if (msgs.length <= 0) {
			Res.log(Res.ERROR, "要发送到消息对象为空！");
			return UMSSendService.RETCODE_SEND_ILLEGAL_XML;
		}
		...
		try {
			...
			String batchNO = Util.getCurrentTimeStr();
	...
	public UMSMsg channelPolicier(UMSMsg msg) throws SQLException {
		...
		if (msg.getBasicMsg().getDirectSendFlag() != BasicMsg.BASICMSG_DIRECTSEND_YES) {
			ChannelPolicy
					.implChannelPolicy(msg, Policy.POLICY_NEXT_PRIORITY_NO);
		}
		if (msg.getBasicMsg().getDirectSendFlag() == BasicMsg.BASICMSG_DIRECTSEND_YES
				&& msg.getBasicMsg().getMediaID() == null) {
	...
	public UMSMsg messagePolicier(BasicMsg basicMsg) {
		...
		if (filterFlag) {
			...
			msg.setBatchNO(Util.getCurrentTimeStr());
			Res.log(Res.WARN, "被过滤消息，UMS阻拦. 内容：" + content);
		}
