...
public class BasicMsg implements Serializable {
	 ...
	 * 订阅的消息
	 */
	public static final int BASICMSG_TOPIC_UMS = 3;
	...
	/**
	 * 深度克隆，拷贝一个完全一样的对象，但属性都指向不同的对象
	 * 
	 * @return
	 */
	public BasicMsg deepClone() {
		BasicMsg basic = new BasicMsg();
		basic.setAck(ack);
		basic.setAppSerialNO(appSerialNO);
		basic.setCompanyID(companyID);
		basic.setContentMode(contentMode);
		basic.setDirectSendFlag(sendDirectly);
		basic.setFeeServiceNO(feeServiceNO);
		basic.setInvalidDate(invalidDate);
		basic.setInvalidTime(invalidTime);
		basic.setMediaID(mediaID);
		if (msgAttachment != null) {
			MsgAttachment[] clonedAtt = new MsgAttachment[msgAttachment.length];
			for (int i = 0; i < msgAttachment.length; i++) {
				clonedAtt[i] = new MsgAttachment(
						msgAttachment[i].getFileName(), msgAttachment[i]
								.getFileByteBase64());
			}
			basic.setMsgAttachment(clonedAtt);
		}
		if (msgContent != null) {
			MsgContent cloendContent = new MsgContent(msgContent.getSubject(),
					msgContent.getContent());
			basic.setMsgContent(cloendContent);
		}
		basic.setNeedReply(needReply);
		basic.setPriority(priority);
		if (receivers != null) {
			Participant[] clonedReceives = new Participant[receivers.length];
			for (int i = 0; i < receivers.length; i++) {
				clonedReceives[i] = new Participant(receivers[i]
						.getParticipantID(), receivers[i].getParticipantType(),
						receivers[i].getIDType());
			}
			basic.setReceivers(clonedReceives);
		}

		basic.setReplyDestination(replyDestination);
		if (sender != null) {
			Participant cloendSender = new Participant(sender
					.getParticipantID(), sender.getParticipantType(), sender
					.getIDType());
			basic.setSender(cloendSender);
		}
		basic.setServiceID(serviceID);
		basic.setSetSendDate(setSendDate);
		basic.setSetSendTime(setSendTime);
		basic.setSubmitDate(submitDate);
		basic.setSubmitTime(submitTime);
		basic.setTimeSetFlag(timeSetFlag);
		basic.setUmsFlag(umsFlag);
		return basic;
	}
}
