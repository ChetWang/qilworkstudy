...
public class NonRectanglePopupFactory extends PopupFactory{
    ...
    class NonRectangleBorder implements Border{
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
            ...
            Rectangle innerBounds=new Rectangle(bounds.x+BORDER_PAD, bounds.y+BORDER_PAD, width-2*BORDER_PAD, height-2*BORDER_PAD);
        ...
        private void generateTopImage(BufferedImage backgroundImage) {
            topImage=backgroundImage.getSubimage(BORDER_PAD, 0, backgroundImage.getWidth()-2*BORDER_PAD, BORDER_PAD);
        }
        
        private void generateRightImage(BufferedImage backgroundImage) {
            rightImage=backgroundImage.getSubimage(backgroundImage.getWidth()-BORDER_PAD, 0, BORDER_PAD, backgroundImage.getHeight());
        }
        
        private void generateBottomImage(BufferedImage backgroundImage) {
            bottomImage=backgroundImage.getSubimage(BORDER_PAD, backgroundImage.getHeight()-BORDER_PAD, backgroundImage.getWidth()-2*BORDER_PAD, BORDER_PAD);
        }
        ...
        private Polygon calculateArrowPolygon(Point cp, Point op, Rectangle outer, Rectangle inner, Point zero) {
            ...
            if(p1==null){
                p1=MathUtil.intersectWithHorizontalLine(cp, op, inner.x,inner.x+inner.width,inner.y+inner.height);
                if(p1==null){
                    p1=MathUtil.intersectWithVerticalLine(cp, op, inner.x,inner.y,inner.y+inner.height);
                    if(p1==null){
                        p1=MathUtil.intersectWithVerticalLine(cp, op, inner.x+inner.width,inner.y,inner.y+inner.height);
                        if(p1!=null)
