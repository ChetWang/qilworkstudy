...
public class DataBaseOp {
	...
	private static UMSPoolingDriver poolDriver;
	private static void setupDriver() {
		try {
			...
			//
			poolDriver = new UMSPoolingDriver();

			//
			// ...and register our pool with it.
			//
			poolDriver.registerPool(pool_name, connectionPool);
//			driver.g
			DriverManager.registerDriver(poolDriver);
	...
	public static void unRegisterDriver(){
		try {
			poolableConnectionFactory.destroyObject(connectionPool);
			poolDriver.getPool().remove(pool_name);
			pool_name = null;			
		} catch (Exception e) {
			Res.log(Res.ERROR, "卸载数据库连接池失败");
