...
//        }
    	...
    	BasicMsg[] msgs = new BasicMsg[1];
    	msgs[0] = new BasicMsg();
    	TextMessage message = session.createTextMessage(new UMSXMLSerializer().serialize(msgs));
    	message.setJMSMessageID(UMSUID.getUMSUID(UMSUID.UMSUID_SERVER));
    	message.setJMSReplyTo(session.createQueue("client_server"));
    	producer.send(message);
    }
