 ...
 * @author Qil.Wong
 */
public abstract class OutChannel extends DefaultChannelIfcImpl implements
		Runnable {
	...
	public OutChannel(MediaInfo mediaInfo) {
		...
		sql_queryUMSMsgs = DialectUtil.getDialect().getOutMsgSelectSQL_V3();
		// eui = new UUMUserInfo();
		...
		if (!isLocked()) {
			...
			try {
				...
				PreparedStatement attPrep = conn
						.prepareStatement("SELECT * FROM UMS_ATTACHMENTS WHERE BATCHNO = ? AND SERIALNO = ? AND SEQUENCENO =?");
			...
			if (repeatMsgCheckFlag) {
				...
				try {
					...
					PreparedStatement prep = conn
							.prepareStatement("select count(*) from UMS_SEND_OK where content=? and recvid=? and batchno>? and retCode=?");
	 ...
	 * UMS_SEND_READY table and put it into UMS_SEND_OK table, if it is a reply
	...
	protected void OpData(int result, UMSMsg msg) throws IOException {
		...
		try {
			String deleteOutReadySQL = DBUtil_V3
					.createDeleteMsgSQL(DBUtil_V3.MESSAGE_UMS_SEND_READY);
			...
			if (result == 0 || result == -9922) {
				// 发送成功
				// delete message from UMS_SEND_READY
				PreparedStatement msgDeletePrep = conn
						.prepareStatement(deleteOutReadySQL);
				// meanwhile, insert this message into UMS_SEND_OK
				PreparedStatement msgPrep = conn
...
//				ChannelPolicy channelPolicy = ChannelPolicy.getInstance();
				...
				if (msg.getRep() >= 3) {
					...
					ChannelPolicy.implChannelPolicy(msg, true);
					try {
						com.nci.ums.tactics.ChannelPolicy.implChannelPolicy(
								msg, Policy.POLICY_NEXT_PRIORITY_YES);
					} catch (Exception e) {
						Res.logExceptionTrace(e);
						e.printStackTrace();
					}
