...
public class Res {
	...
	private static Logger logger = Logger.getLogger(Res.class);
	...
	/* get config infomation of bip system */
	private static String CONFIG = "/resources/config.xml";
	...
	private static ServletTemp servletTemp = new ServletTemp();
	// private static ArrayList umsThreads;
	static {
		/* Load code properties */
		res = new Properties(); // Empty properties
		try {
			InputStream in = servletTemp.getInputStreamFromFile(props);
			// InputStream in = ClassLoader.getSystemResourceAsStream(props);
			// InputStream in = new
			// Object().getClass().getResourceAsStream(props);
		...
		Appender textAreaAppender = DesktopConsole.getComponentAppender();
		if (textAreaAppender != null) {
			logger.addAppender(textAreaAppender);
			textAreaAppender.setLayout(layout);
		}
	...
	public static void logExceptionTrace(Exception e) {
		// synchronized (syncLock) {
		Res.log(Res.ERROR, e.getClass()+":"+e.getMessage());
