...
public class NonRectanglePopupFactory extends PopupFactory {
	...
	class NonRectangleFrame extends JComponent {
		public NonRectangleFrame(Component owner, Component content,
			...
			NonRectangleBorder border = new NonRectangleBorder(owner, content,
					backgroundImage);
			setBorder(border);
		}
	}
	...
	class NonRectangleBorder implements Border {
		...
		public void paintBorder(Component c, Graphics g, int x, int y,
			...
			g
					.drawImage(bottomImage, x + BORDER_PAD, y + height
							- BORDER_PAD, c);
			Point op = owner.getLocationOnScreen();
			Point cp = c.getLocationOnScreen();
			Point ccp = new Point(cp.x + c.getWidth() / 2, cp.y + c.getHeight()
					/ 2);
			op.x += owner.getWidth() / 2;
			op.y += owner.getHeight() / 2;
			Rectangle bounds = new Rectangle(cp.x + x, cp.y + y, width, height);
			Rectangle innerBounds = new Rectangle(bounds.x + BORDER_PAD,
					bounds.y + BORDER_PAD, width - 2 * BORDER_PAD, height - 2
							* BORDER_PAD);
			Polygon polygon = calculateArrowPolygon(ccp, op, bounds,
					innerBounds, cp);
		...
		private void generateBottomImage(BufferedImage backgroundImage) {
			bottomImage = backgroundImage.getSubimage(BORDER_PAD,
					backgroundImage.getHeight() - BORDER_PAD, backgroundImage
							.getWidth()
							- 2 * BORDER_PAD, BORDER_PAD);
		}

		private Polygon calculateArrowPolygon(Point cp, Point op,
				Rectangle outer, Rectangle inner, Point zero) {
			Polygon polygon = new Polygon();
			Point p2 = MathUtil.intersectPoint(op, cp, outer);
			if (p2 != null)
				polygon.addPoint(p2.x, p2.y);
			Point p1 = MathUtil.intersectWithHorizontalLine(cp, op, inner.x,
					inner.x + inner.width, inner.y);
			if (p1 == null) {
				p1 = MathUtil.intersectWithHorizontalLine(cp, op, inner.x,
						inner.x + inner.width, inner.y + inner.height);
				if (p1 == null) {
					p1 = MathUtil.intersectWithVerticalLine(cp, op, inner.x,
							inner.y, inner.y + inner.height);
					if (p1 == null) {
						p1 = MathUtil.intersectWithVerticalLine(cp, op, inner.x
								+ inner.width, inner.y, inner.y + inner.height);
						if (p1 != null)
		...
		private void addTopBorderPoint(Rectangle inner, Polygon polygon, Point p) {
			...
			polygon.addPoint(inner.x + inner.width + 1, inner.y + inner.height
					+ 1);
		...
		private void addBottomBorderPoint(Rectangle inner, Polygon polygon,
				Point p) {
			if (p.x + ARROW_PAD < inner.x + inner.width + 1)
		...
		private void addLeftBorderPoint(Rectangle inner, Polygon polygon,
				Point p) {
		...
		private void addRightBorderPoint(Rectangle inner, Polygon polygon,
				Point p) {
			if (p.y + ARROW_PAD < inner.y + inner.height + 1)
