...
public class AppletProxy {
	...
	public String getSvgContent() {
		try {
			...
			if (applet.getInitFlag() == DrawSvg.INIT_COMPLETE) {
				...
				System.out.println("DrawSvg is initializing..., getSVGContent failed!");
			}
		} finally {
	...
	public void openFileByCode(String strSapCode) {
		// 从功能位置码打开文件
		if (applet.getInitFlag() == DrawSvg.INIT_COMPLETE)
			editor.getSvgSession().openRemoteSVGFileOldByCode("psms",
					strSapCode);

	}

	public String saveJpegFile(final DrawSvg applet, final String fileName) {

		System.out.println("saveJpegFile begin");
		Document document = editor.getHandlesManager().getCurrentHandle()
				.getCanvas().getDocument();
		SVGDocumentImageCreator imageCreator = new SVGDocumentImageCreator(
				document, null, editor);

		// adding the listener that will be called when the image is created
		SVGDocumentImageCreatorListener listener = new SVGDocumentImageCreatorListener() {

			@Override
			public void imageCreated(BufferedImage image) {
				System.out.println("imageCreated ok");
				File file = new File(
						Constants.NCI_SVG_CACHE_CLIP_TEMP_CACHE_DIR + "2.jpg");
				writeImage(image, file);

				// File file = new File(fileName);
				// applet.writeImage(image, file);
				try {
					// FileInputStream stream = new FileInputStream(file);
					// byte[] b = new byte[stream.available()];
					// stream.read(b);
					byte[] b = image_byte_data(image);

					String content = applet.getByteHexStr(b, 0, b.length);

					String[][] params = new String[][] {
							{ "filename", fileName }, { "content", content } };
					String url = (String) editor.getGCParam("appRoot")
							+ (String) editor.getGCParam("servletPath")
							+ "?action=saveOneJpegfileOld";
					String ret = applet.getUrlRetText(url, params);
		...
		int width = editor.getHandlesManager().getCurrentHandle().getCanvas()
				.getWidth();
		int height = editor.getHandlesManager().getCurrentHandle().getCanvas()
				.getHeight();
		imageCreator.createImage(new Dimension((int) width, (int) height),
				false);
		return "0";
	}
	...
	public void openSVGFile(String filename) {
		if (applet.getInitFlag() == DrawSvg.INIT_COMPLETE)
			editor.getSvgSession().openRemoteSVGFileOld("", filename);
	...
	public void openSVGFile(String fileType, String fileRemark,
			String voltageClass) {
		if (applet.getInitFlag() == DrawSvg.INIT_COMPLETE)
			editor.getSvgSession().openRemoteSVGFile(fileType, fileRemark,
					voltageClass);
	}
	...
	public void openFile(String strFileName) {
		if (applet.getInitFlag() == DrawSvg.INIT_COMPLETE)
			editor.getSvgSession().openRemoteSVGFileOld("", strFileName);
	}
	...
	public void openSVGFile(String lineID, String fileType, String fileRemark,
			String voltageClass) {
		if (applet.getInitFlag() == DrawSvg.INIT_COMPLETE)
			editor.getSvgSession().openRemoteSVGFile(lineID, fileType,
					fileRemark, voltageClass);
	}
	...
	public void openFileByLineID(String strLineID, int fileType, int type) {
		editor.getSvgSession().openFileByLineID(strLineID, fileType, type);
		return;
	}
	...
	public void openFileByMisID(String misID){
		editor.getSvgSession().openFileByMisID(misID);
		return;
	}
