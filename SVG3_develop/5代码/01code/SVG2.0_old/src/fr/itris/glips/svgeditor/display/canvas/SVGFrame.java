...
import com.nci.svg.ui.graphunit.NCIGraphUnitSelectionPanel;
...
public class SVGFrame extends JInternalFrame {
	...
	/**
	 * the graph unit selection bar
	 */
	NCIGraphUnitSelectionPanel unitSelectionPanel;
	...
	public SVGFrame(SVGHandle svgHandle) {
		...
		super("", true, svgHandle.getEditor().getModeManager().isSVGFrameCloseable(), true, true);
	...
	/**
	 * 创建图元管理界面，added by wangql
	 */
	protected void buildGraphUnitManageView() {
		...
		// added by wangql, creating and adding the graph unit selection panel
		unitSelectionPanel = new NCIGraphUnitSelectionPanel(svgHandle);
		unitSelectionPanel.setSize(600, 350);
		contentPane.add(unitSelectionPanel, BorderLayout.NORTH);
		// wangql added end
	...
	public void setSVGFrameLabel(String name) {
		...
		if (svgHandle.getHandleType() == SVGHandle.HANDLE_TYPE_GRAPH_UNIT_OLD
				|| svgHandle.getHandleType() == SVGHandle.HANDLE_TYPE_GRAPH_UNIT_BAY
				|| svgHandle.getHandleType() == SVGHandle.HANDLE_TYPE_GRAPH_UNIT_NORMAL) {
			// if (!label.equals(ResourcesManager.bundle
			// .getString(NCIGraphUnitModule.idGraphUnitManage)))
			// label = ResourcesManager.bundle
			// .getString(NCIGraphUnitModule.idGraphUnitManage)
			// + ": " + label.trim();
			if (svgHandle.getGraphUnit() == null) {
				label = ResourcesManager.bundle
						.getString(NCIGraphUnitModule.idGraphUnitManage)
						+ ":  " + label.trim();
			} else {
				label = ResourcesManager.bundle
						.getString(NCIGraphUnitModule.idGraphUnitManage)
						+ ":  "
						+ svgHandle.getGraphUnit().getGraphUnitType()
						+ "/" + svgHandle.getGraphUnit().getGraphUnitName();
			}
		}
