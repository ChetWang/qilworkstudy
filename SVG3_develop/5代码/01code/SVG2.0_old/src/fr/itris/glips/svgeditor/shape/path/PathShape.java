...
public class PathShape extends AbstractShape {
    ...
    private static String currentAction = "LineShape";
    /**
     ...
     * 是否是连接线
     */
    private boolean isConnectedPath = false;
    ...
    public static void setCurrentAction(String currentAction) {
        PathShape.currentAction = currentAction;
    }
    ...
    public void notifyItemsAction(final SVGHandle handle, SelectionItem item) {
        ...
        executeRunnable = new Runnable() {
            ...
            public void run() {
                ...
                module.correctTextPos(handle, element,initDValue,modifiedDValue);
            }
        };
        ...
        undoRunnable = new Runnable() {
            ...
            public void run() {
                ...
                NciTextPathModule module = new NciTextPathModule(editor);
                module.correctTextPos(handle, element,modifiedDValue,initDValue);
            }
        };
...
//        resetDrawing();
     ...
     */
    public Element createElement(SVGHandle handle, ExtendedGeneralPath pathShape) {
        ...
        String strLineID = cID.getLineID();
        element.setAttribute("id", strLineID);
    ...
    protected UndoRedoAction applyTransform(final SVGHandle handle,
        ...
        final boolean bSingleFlag = (handle.getSelection()
                .getSelectedElements().size() > 1 ? false : true);
        // setting the new translation factors
        Runnable executeRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (bSingleFlag) {
                    UpholdElementRela rela = new UpholdElementRela(handle
                            .getCanvas().getLpManager());
                    rela.upholdElement(handle, element,
                            UpholdElementRela.MODIFY_TYPE);
                }
                NciTextPathModule module = new NciTextPathModule(editor);
...
//            textModule.correctTextPos(handle, element, initdValue, dValue);
    ...
    public Set<SelectionItem> getModifyPointsSelectionItems(SVGHandle handle,
        ...
        // creating the items
        fr.itris.glips.library.geom.path.segment.Segment segment = shape.getSegment();
