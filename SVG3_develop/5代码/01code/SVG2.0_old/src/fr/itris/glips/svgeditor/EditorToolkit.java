...
public class EditorToolkit {
	...
	public Cursor createCursorImageFromColor(Color color) {
		...
		if (color != null) {
			...
			Dimension bestSize = Toolkit.getDefaultToolkit().getBestCursorSize(
					22, 22);

			try {
				cursor = Toolkit.getDefaultToolkit().createCustomCursor(
						getImageFromColor(color, bestSize), new Point(0, 0),
						"color");
			} catch (Exception ex) {
			}
		}
	...
	public static void setStyleProperty(Element element, String name,
		...
		if (element != null && name != null && !name.equals("")) {
			...
			String styleValue = element.getAttribute("style");
			Utilities.printNode(element, false);
			styleValue = styleValue.replaceAll("\\s*[;]\\s*", ";");
			styleValue = styleValue.replaceAll("\\s*[:]\\s*", ":");
