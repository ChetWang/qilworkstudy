...
public class ClipboardModule extends ModuleAdapter {
	 ...
	 *            the editor
	 */
	public ClipboardModule(final Editor editor) {
		super(editor);
	...
	public void copy() {
		...
		if (handle != null) {
			...
			final LinkedList<Element> docChildNodesList = Toolkit
					.getChildrenElements(parent);

			// creating the linkedlist of the elements to be handled
			final LinkedList<Element> elementsList = new LinkedList<Element>();
			// added by wangql--start
			// Document currentDoc = Editor.getEditor().getHandlesManager()
			// .getCurrentHandle().getCanvas().getDocument();
			// Element tempCopyGroup =
			// currentDoc.createElementNS(currentDoc.getNamespaceURI(), "g");
			// Iterator<Element> it = selectedNodes.iterator();
			// while(it.hasNext()){
			// tempCopyGroup.appendChild(it.next());
			// }
			// elementsList.add(tempCopyGroup);
			// added by wangql-- end
			// Set<Element> clonedSelectedNodesSet = new HashSet<Element>(
			// selectedNodes);
			// for(Element element :clonedSelectedNodesSet)
			// {
			...
			// if(textElement != null &&
			// !clonedSelectedNodesSet.contains(textElement))
			// {
			// selectedNodes.add(textElement);
			// }
			// }
			// }
			...
			// {
			// Element pathElement = handle.getCanvas().getMapInfo(strPathID);
			// if(pathElement != null &&
			// !clonedSelectedNodesSet.contains(pathElement))
			// {
			// selectedNodes.add(pathElement);
			// }
			// }
			// }
			// }
			// deprecated by wangql
				...
				final Runnable executeRunnable = new Runnable() {
					...
					public void run() {
						...
						for (Element current : nodesToPaste) {
							...
							if (current != null) {								
								parseUseSymbol(handle, defs, current, parent);
				...
				final Runnable undoRunnable = new Runnable() {
					...
					public void run() {
						...
						for (Element current : nodesToPaste) {
							...
							if (current != null) {
								String strID = current.getAttribute("id");
...
//				pasteIndex++;				
	...
	public void delete(boolean isDelete) {
		...
		if (selectedNodes.size() > 0) {
			...
			final LinkedList<Element> elementsToDelete = new LinkedList<Element>();
				...
				public void run() {
					...
					for (Element element : elementsToDelete) {
						...
						if (element != null) {
							...
							String strID = element.getAttribute("id");
							if (strID != null && strID.length() > 0) {
								handle.getCanvas().putMapInfo(strID, element);
							}
