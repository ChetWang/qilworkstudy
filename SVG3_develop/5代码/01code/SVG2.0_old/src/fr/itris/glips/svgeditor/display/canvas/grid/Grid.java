    ...
    protected void initializeGrid() {
        ...
        CanvasPainter paintListener = new CanvasPainter() {
            ...
            public void paintToBeDone(Graphics2D g) {
                ...
                GridParametersManager handler = canvas.getSVGHandle().getEditor().
                        getHandlesManager().getGridParametersHandler();

                if (handler.isGridEnabled()) {

                    //getting the grid parameters
//                    double horizontalDistance = handler.getHorizontalDistance(canvas.getSVGHandle().getHandleType());
//                    double verticalDistance = handler.getVerticalDistance(canvas.getSVGHandle().getHandleType());
                    /**add and changed by wangql**/
                    double horizontalDistance = handler.getHorizontalDistance(canvas.getSVGHandle().getHandleType());
                    double verticalDistance = handler.getVerticalDistance(canvas.getSVGHandle().getHandleType());
