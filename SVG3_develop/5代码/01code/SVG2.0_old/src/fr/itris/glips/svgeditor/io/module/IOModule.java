...
public class IOModule extends ModuleAdapter {
	 ...
	 */
	protected String[] ids = { "FileNew", "FileOpen", "FileOpenPersonal","FileOpenRecent",
			"FileSave", "FileSaveToServer", "FileSaveAs", "FileExport",
			"FilePrint", "FileClose", "EditorExit","FileOpenStandard" };
	protected String[] accelerators = { "ctrl N", "ctrl O", null,null, "ctrl S",
			null, "ctrl shift S", null, "ctrl P", "ctrl W", "atl F4" ,null};
	/**
	...
	public IOModule(Editor editor) {
		super(editor);
		openRecentMenuManager = new OpenRecentMenuManager(editor);
		exportMenuManager = new ExportMenuManager(this);
		fileNewMenuManager = new FileNewMenuManager(this);
		createMenuItems();
		...
		ActionListener exitListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				new Thread(new Runnable() {
					public void run() {
						editor.getIOManager()
								.getEditorExitManager().exit();
//						Utilities.openEditor();
					}
				}).start();
	...
	protected void doAction(int index) {
		...
		case 0:
			...
			editor.getIOManager().getFileNewManager()
			...
			if (currentHandle.getHandleType() == SVGHandle.HANDLE_TYPE_SVG
				...
				editor.getIOManager().getFileSaveManager()
						.saveHandleDocumentToServer(currentHandle,
								menuItems[index]);
			} else if (currentHandle.getHandleType() == SVGHandle.HANDLE_TYPE_GRAPH_UNIT_NORMAL) {
				editor.getIOManager().getFileSaveManager()
						.saveGraphUnitHandleDocToServer(currentHandle,
								menuItems[index]);
			} else {
		...
		case 6:
			...
			case SVGHandle.HANDLE_TYPE_SVG:
				editor.getIOManager().getFileSaveManager()
						.saveHandleDocument(currentHandle, true,
								menuItems[index]);
				break;
			}
			break;
		case 8:
			...
			editor.getIOManager().getFilePrint().print(
					currentHandle);
			break;

		case 9:

			editor.getIOManager().getFileCloseManager()
					.closeHandle(currentHandle, menuItems[index]);
			break;
		...
		case 10:
			SwingWorker<Object, Object> w = new SwingWorker<Object, Object>() {
				...
				protected Object doInBackground() throws Exception {
					editor.getIOManager().getEditorExitManager()
							.exit();
					return null;
				}
			};
