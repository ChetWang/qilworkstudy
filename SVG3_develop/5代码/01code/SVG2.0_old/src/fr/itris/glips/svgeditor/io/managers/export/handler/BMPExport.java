...
public class BMPExport extends Export{
	...
	protected BMPExport(FileExport fileExport) {
		...
		if(getFileExport().getEditor().getParent() instanceof Frame){
			...
			exportDialog=new BMPExportDialog((Frame)fileExport.getEditor().getParent(),fileExport.getEditor());
			
		}else{
			
			exportDialog=new BMPExportDialog((JDialog)fileExport.getEditor().getParent(),fileExport.getEditor());
		}
	}
	...
	public void export(JComponent relativeComponent, 
		...
		monitor=new ExportMonitor(
				getFileExport().getEditor().getParent(), 0, 100, FileExport.prefixLabels[2]);
