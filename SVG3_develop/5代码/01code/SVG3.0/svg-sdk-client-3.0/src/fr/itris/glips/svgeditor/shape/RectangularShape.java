...
public abstract class RectangularShape extends AbstractShape {
	...
	/**
	 * the painter drawing ghost rectangles on the
	 * canvas，绘制虚图的绘制器,指从面板中拖拽过来的基本图元或自定义图元在未完全绘制在画布前的形状。
	 */
	protected GhostShapeCanvasPainter ghostCanvasPainter = new GhostShapeCanvasPainter();

	/**
	 * the label for the undo/redo action of the modification of the radius
	 * attribute of a rectangle. 修改后的标记
	 */
	 ...
	 *            the editor
	 */
	public RectangularShape(EditorAdapter editor) {
	...
	/**
	 * creates a rectangular element by specifiying its bounds。 创建一个类“矩形”框架
	 * 
	 * @param handle
		...
		if (nSize > 0) {
			for (int i = 0; i < nSize; i++) {
				...
				items.add(new SelectionItem(handle, elements, handle
						.getTransformsManager().getScaledPoint(pp, false),
						SelectionItem.POINT, SelectionItem.POINT_STYLE, 0,
						false, null));
	...
	public UndoRedoAction translate(final SVGHandle handle,
		...
		final java.awt.geom.RectangularShape initialShape = (java.awt.geom.RectangularShape) getShape(
				handle, element, false);

		if (initialShape != null) {

			// getting the translation factors
			Point2D translationCoefficients = ShapeToolkit
					.getTranslationFactors(handle, element, translationFactors
							.getX(), translationFactors.getY());

			// creating the new shape
			final java.awt.geom.RectangularShape newShape = new Rectangle2D.Double(
			...
			undoRedoAction = ShapeToolkit.getUndoRedoAction(
					translateUndoRedoLabel, executeRunnable, undoRunnable,
					elements);
		}
	...
	public UndoRedoAction resize(final SVGHandle handle,
			Set<Element> elementSet, final AffineTransform transform) {
		...
		if (initialTransform.isIdentity()) {
			...
			final java.awt.geom.RectangularShape newShape = transform
	...
	/**
	 * returns a undo/redo action used to modify the radius attribute for the
	 * element
	 * 
	 * @param handle
	 ...
	 * @param radius
	 *            the new radius for the element
	...
	public UndoRedoAction modifyRadius(final SVGHandle handle,
		...
		elements.add(element);
		UndoRedoAction undoRedoAction = ShapeToolkit.getUndoRedoAction(
				radiusModificationUndoRedoLabel, executeRunnable, undoRunnable,
				elements);
	...
	public Shape getShape(SVGHandle handle, Element element, boolean isOutline) {
		...
		if (!Double.isNaN(initX) && !Double.isNaN(initY)
			...
			rectangle = new RoundRectangle2D.Double(initX, initY, initW, initH,
					2 * initRX, 2 * initRY);
		}
	...
	protected Point2D getRadius(SVGHandle handle, Element element,
		...
		RoundRectangle2D rect = (RoundRectangle2D) getShape(handle, element,
				false);

		// getting the previous factors
		Point2D.Double factors = new Point2D.Double(rect.getArcWidth() / 2,
				rect.getArcHeight() / 2);

		// transforming the provided point with the inverse transform of the
		// element,
		// so that the point that is found is untransformed
		AffineTransform elementAf = handle.getSvgElementsManager()
				.getTransform(element);
		...
		if (elementAf != null && !elementAf.isIdentity()) {
			...
			try {
				...
				if (inverseTransform != null) {
					...
					currentPoint = inverseTransform.transform(currentPoint,
							null);
				}
			} catch (Exception ex) {
			}
		...
		// getting the new factor for the axis defined by the provided selection
		// item
		double factor = 0;
		...
		case 0:
			...
			factors = new Point2D.Double(factor, factors.getY() == 0 ? factor
					: factors.getY());
			break;
		...
		case 1:
			...
			// creating the point that will be returned
			factors = new Point2D.Double(factors.getX() == 0 ? factor : factors
					.getX(), factor);
	...
	/**
	 * returns the modified shape for the provided element, without its
	 * element's transform applied with its radius factors modified.
	 ...
	 *            an element
	 * @param newFactors
	 *            the new radius factors for the rectangle
	 * @return the transformed shape for the provided element with its radius
	 *         factors modified
	 */
	protected Shape getModifiedRadiusShape(SVGHandle handle, Element element,
			Point2D newFactors) {
		...
		rect.setRoundRect(rect.getX(), rect.getY(), rect.getWidth(), rect
				.getHeight(), newFactors.getX() * 2, newFactors.getY() * 2);

		return rect;
	}

	/**
	 * returns the set of the selection items corresponding to a radius
	 * modification selection level and for the elements and the covered area
	 * 
	 * @param handle
	 *            a svg handle
	 * @param elements
	 ...
	 * @return the set of the selection items corresponding to a radius
	 *         modification selection level and for the elements and the covered
	 *         area
	 */
	public Set<SelectionItem> getRadiusSelectionItems(SVGHandle handle,
		...
		// scaling each point
		xRadiusPoint = handle.getTransformsManager().getScaledPoint(
				xRadiusPoint, false, transform);
		yRadiusPoint = handle.getTransformsManager().getScaledPoint(
				yRadiusPoint, false, transform);
		...
		items.add(new SelectionItem(handle, elements, yRadiusPoint,
				SelectionItem.POINT, SelectionItem.CENTER_POINT_STYLE, 1,
				false, null));
		...
		public void paintToBeDone(Graphics2D g) {
			...
			if (shape != null) {
				...
				if (shape instanceof RoundRectangle2D) {
					...
					g.drawRoundRect((int) roundRect.getX(), (int) roundRect
							.getY(), (int) roundRect.getWidth(),
							(int) roundRect.getHeight(), (int) roundRect
									.getArcWidth(), (int) roundRect
									.getArcHeight());
