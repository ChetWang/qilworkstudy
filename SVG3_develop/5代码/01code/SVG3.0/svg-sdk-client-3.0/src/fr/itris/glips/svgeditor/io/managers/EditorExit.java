...
public class EditorExit {
	...
	public void exit() {
		...
		if (ioManager.getEditor().isQuitActionDisabled()) {
			...
			Collection<SVGHandle> handles = new LinkedList<SVGHandle>(ioManager
					.getEditor().getHandlesManager().getHandles());
			...
			if (displayDialog) {
				...
				int returnVal = JOptionPane.showConfirmDialog(ioManager
						.getEditor().getParent(), warningMessage, warningTitle,
			...
			if (canExitEditor) {
				...
				try {
					((HelpModuleAdapter) ioManager.getEditor().getModule(
							HelpModuleAdapter.MODULE_ID)).closeHelp();
				} catch (Exception e) {
