...
public abstract class MultiAbstractShape extends AbstractShape {
	 ...
	 *            the editor
	 */
	public MultiAbstractShape(EditorAdapter editor) {
	...
	/**
	 * returns the set of the elements that should be handled according to the
	 * provided set
	 * 
	 * @param elements
	 *            a set of elements
	 * @return the set of the elements that should be handled according to the
	 *         provided set
	 */
	...
	/**
	 * returns the bounds corresponding to the union of the bounds of each
	 * element
	 * 
	 * @param handle
	 ...
	 * @param elements
	 *            a set of elements
	 * @return the bounds corresponding to the union of the bounds of each
	 *         element
	 */
	 ...
	 *         element
	 */
	public static Rectangle2D getElementsBounds(SVGHandle handle,
			Set<Element> elements) {

		Rectangle2D wholeBounds = null;
		Rectangle2D elementBounds = null;

		// getting the bounds of each element and merging it to the already
		// computed area
	...
	public UndoRedoAction validateTranslateAction(SVGHandle handle,
		...
		Point2D translationFactors = new Point2D.Double(currentPoint.getX()
				- firstPoint.getX(), currentPoint.getY() - firstPoint.getY());

		UndoRedoAction urAction = translate(handle, elementSet,
				translationFactors, true);
		return getUndoRedoAction(handle, urAction, elementSet);
	}
	...
	public CanvasPainter showAction(SVGHandle handle, int level,
		...
		// creating the union of all the shapes corresponding to the set of
		// elements
		...
		if (unionShape != null) {
			...
			case 0:
				...
				actionTransform = getResizeTransform(handle, bounds, item,
			...
			case 1:
				...
				if (item.getType() == SelectionItem.CENTER) {
					...
					actionTransform = getRotationTransform(handle, bounds,
							firstPoint, currentPoint);

				} else {

					// getting the skew transform
					actionTransform = getSkewTransform(handle, bounds,
			...
			if (canPaintShape && actionTransform != null) {
				...
				// computing the screen scaled shape
				Shape shape = handle.getTransformsManager().getScaledShape(
						unionShape, false, actionTransform);
	...
	public UndoRedoAction validateAction(SVGHandle handle, int level,
			Set<Element> elementsSet, SelectionItem item, Point2D firstPoint,
			Point2D lastPoint) {
		...
		case 0:
			...
			AffineTransform resizeTransform = getResizeTransform(handle,
					bounds, item, firstPoint, lastPoint);
		...
		case 1:
			...
			if (item.getType() != SelectionItem.CENTER) {
				...
				if (item.getType() == SelectionItem.NORTH_WEST
					...
					undoRedoAction = rotate(handle, elementsSet, centerPoint,
							angle);
					...
					undoRedoAction = skew(handle, elementsSet, centerPoint,
		...
		if (undoRedoAction == null)
			return null;
		return getUndoRedoAction(handle, undoRedoAction, elementsSet);
	}

	/**
	 * returns the shape union of the shapes corresponding to each element in
	 * the set
	 * 
	 * @param handle
	 *            a svg handle
	 * @param elementsSet
	 *            a set of elements
	 * @return the shape union of the shapes corresponding to each element in
	 *         the set
	 */
	protected Shape getTransformedShape(SVGHandle handle,
	...
	/*
	 * protected Shape getTransformedShape(SVGHandle handle, Set<Element>
	 * elementsSet){
	 * 
	 * //getting the set of the elements that should be handled
	 ...
	 * 
	 * //creating the union of all the shapes corresponding to the set of
	 * elements GeneralPath path=new GeneralPath(); Area area=null; LinkedList<Shape>
	 * lineShapes=new LinkedList<Shape>(); AbstractShape shapeModule=null;
	 * Shape shp=null;
	 * 
	 * for(Element element : elementsSet){
	 * 
	 ...
	 * if(area!=null){
	 * 
	 * //filling the path path.append(area, false); }
	 * 
	 * //adding the line shapes to the path for(Shape lineShape : lineShapes){
	 * 
	...
	public UndoRedoAction translate(SVGHandle handle, Set<Element> elementSet,
		...
		// translating each element and registering the returned undo/redo
		// action
		...
		for (Element element : elementSet) {
			...
			if (shapeModule != null) {
				...
				action = shapeModule.translate(handle, elementsToTransform,
						translationFactors, refresh);
		...
		return getUndoRedoAction(handle, undoRedoActionSet, elementSet,
	...
	public UndoRedoAction resize(SVGHandle handle, Set<Element> elementSet,
		...
		for (Element element : elementSet) {
			...
			shapeModule = ShapeToolkit.getShapeModule(element, editor);
			if (shapeModule instanceof GroupShape) {
			...
			if (shapeModule != null) {
				...
				elementsToTransform = new HashSet<Element>();
				elementsToTransform.add(element);
				undoRedoAction = shapeModule.resize(handle,
						elementsToTransform, transform);
	...
	public UndoRedoAction rotate(SVGHandle handle, Set<Element> elementSet,
		...
		for (Element element : elementSet) {
			...
			if (shapeModule != null) {
				...
				elementsToTransform = new HashSet<Element>();
				elementsToTransform.add(element);
				undoRedoAction = shapeModule.rotate(handle,
						elementsToTransform, centerPoint, angle);
	...
	public UndoRedoAction skew(SVGHandle handle, Set<Element> elementSet,
		...
		for (Element element : elementSet) {
			...
			if (shapeModule != null) {
				...
				elementsToTransform = new HashSet<Element>();
				elementsToTransform.add(element);
				undoRedoAction = shapeModule.skew(handle, elementsToTransform,
	...
	protected UndoRedoAction getUndoRedoAction(SVGHandle handle,
		...
		final UndoRedoAction undoRedoAction = handle
				.getCanvas()
				.getLpManager()
				.validateAction(LinkPointManager.SYMBOL_ACTION_MODIFY, elements);
	...
	protected UndoRedoAction getUndoRedoAction(final SVGHandle handle,
			final UndoRedoAction undoRedoAction, final Set<Element> elementsSet) {
		...
		// executing the action and creating the undo/redo action
		return ShapeToolkit.getUndoRedoAction(undoRedoAction.getName(),
				executeRunnable, undoRunnable, elementsSet);
	}
