...
/**
 * the class of the listener to the key events used for doing actions on
...
public class KeySelectionListener extends KeyAdapter {
		...
		awtEventListener = new AWTEventListener() {
			...
			public void eventDispatched(AWTEvent event) {
				...
				if (event instanceof KeyEvent) {
					...
					if ((keyEvent.getID() == KeyEvent.KEY_RELEASED || keyEvent
							.getID() == KeyEvent.KEY_PRESSED)
							&& KeySelectionListener.this.selection
									.getSVGHandle().getSVGFrame().isSelected()
	...
	public void keyPressed(KeyEvent evt) {
		if (!selection.getSVGHandle().getEditor().getGCParam("mode").equals(
		...
		if (!selection.getSVGHandle().getEditor().getGCParam("mode").equals(
			...
			currentTask = new TimerTask() {
				...
				public void run() {
					...
					// selection.fireGraphicTranslated(selection
					// .getSelectedElements(), null, null);
				}
			};
	...
	/**
	 * returns whether the provided key code denotes a UP, DOWN, LEFT or right
	 * key
	 * 
	 * @param keyCode
	 *            the key code
	 * @return whether the provided key code denotes a UP, DOWN, LEFT or right
	 *         key
	 */
	...
	/**
	 * returns the translation factors corresponding to the provided key code
	 * and delta value
	 * 
	 * @param keyCode
	 *            the key code
	 * @param delta
	 *            the delta value
	 * @return the translation factors corresponding to the provided key code
	 *         and delta value
	 */
					...
					try {
						SwingUtilities.invokeAndWait(new Runnable() {
							...
							public void run() {
								...
								translate(currentKeyCode, (int) Math
										.floor(fcurrentLoopNumber / 2 + 1));
							}
						});
					} catch (Exception ex) {
					}
