...
public class GroupShape extends MultiAbstractShape {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see fr.itris.glips.svgeditor.shape.MultiAbstractShape#showAction(fr.itris.glips.svgeditor.display.handle.SVGHandle,
	 *      int, java.util.Set,
	 ...
	 *            the editor
	 */
	public GroupShape(EditorAdapter editor) {
	...
	public Collection<PopupItem> getPopupItems() {
		...
		PopupItem groupItem = new PopupItem(editor, shapeModuleId, itemLabel,
				shapeModuleId) {
		...
		// creating the group popup item
		PopupItem unGroupItem = new PopupItem(editor, unGroupId, unGroupLabel,
				unGroupId) {
						...
						for (Element element : selectedElements) {
							...
							if (!element.getNodeName().equals(
								...
								if (element.getAttribute("type").equals(
										"solidified")
										||
										element.getAttribute(Constants.SYMBOL_STATUS).equalsIgnoreCase(NCIEquipSymbolBean.SYMBOL_TYPE_TEMPLATE)) {
									unGroupEnabled = false;
									break;
								}
							}
	...
	protected void group(boolean bBayFlag) {
		...
		{
			if(pNode == null)
				pNode = element.getParentNode();
			
			if(!element.getParentNode().equals(pNode))
			{
				editor.getSvgSession().showMessageBox("操作提示", "选中的图形不在同一个层内，不能被组合");
				return;
			}
		}

		// getting the parent node
		final Element parentNode = (Element)pNode;
			...
			public void run() {
				...
				for (Element element : orderedElements) {
					...
					if (selectedElements.contains(element)) {
						...
						currentHandle.getCanvas().deleteElement(element.getAttribute("id"), element);
				...
				currentHandle.getCanvas().appendElement(id, groupElement);
			...
			public void run() {
				...
				for (Element element : orderedElements) {
					...
					if (selectedElements.contains(element)) {
						...
						currentHandle.getCanvas().appendElement(element.getAttribute("id"), element);
					}
				...
				currentHandle.getCanvas().deleteElement(id, groupElement);
			}
		};
...
//				.getParentElement();
		final Element parentNode = (Element)gelement.getParentNode();
				...
				for (Element groupElement : groupNodes.keySet()) {
					...
					for (Element childElement : groupChildNodesList) {
						...
						if (childElement != metadata)
						{
							...
							currentHandle.getCanvas().appendElement(childElement.getAttribute("id"), childElement);
						}
			...
			public void run() {
				...
				for (Element groupElement : groupNodes.keySet()) {
					...
					for (Element childElement : groupChildNodesList) {
						...
						currentHandle.getCanvas().deleteElement(childElement.getAttribute("id"), childElement);
					...
					currentHandle.getCanvas().appendElement(groupElement.getAttribute("id"), groupElement);
				}
			}
		};
			...
			if (currentHandle != null) {
				...
				selectionChangedListener = new SelectionChangedListener() {
					...
					public void selectionChanged(Set<Element> selectedElements) {
						...
						GroupHandlesListener.this.selectionChanged(editor
