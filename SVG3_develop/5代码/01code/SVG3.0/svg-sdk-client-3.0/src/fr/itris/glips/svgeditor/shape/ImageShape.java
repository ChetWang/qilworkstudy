...
public class ImageShape extends RectangularShape {
	...
	protected static final String[] extensions = { "png", "jpg", "jpeg" };
	 ...
	 *            the editor
	 */
	public ImageShape(final EditorAdapter editor) {
...
//	}
	...
	/**
	 * 返回选中的Image标签对象路径。 returns the image file selected by the user to be
	 * inserted in the canvas
	 * 
	 * @param handle
	 *            a svg handle
	 * @return an image file path relative to the canvas path denoted by the
	 *         provided handle
	 */
	public String getImageFile(SVGHandle handle) {
		...
		if (canvasFile != null) {
			...
			fileDialog.askForFile(extensions, initialFile,
					new File[] { canvasFile });
			...
			if (foundFile != null) {
				...
				String canvasPath = canvasFile.getParentFile().toURI()
						.toASCIIString();
				foundFilePath = Toolkit.getRelativePath(foundFile.toURI()
						.toASCIIString(), canvasPath);
	 ...
	 * @return
	 */
	public String getImageBase64(SVGHandle handle) {
		...
		String contentBase64 = "";
			...
			// displaying the file chooser dialog
			fileDialog.askForFile(extensions, new File(System.getProperty("user.home")),
					new File[] { });

			// getting the selected file
			foundFile = fileDialog.getLastSelectedFile();
			if (foundFile != null) {
				fileType = foundFile.getName().substring(
						foundFile.getName().lastIndexOf(".") + 1);
				try {
					BufferedInputStream bis = new BufferedInputStream(
							new FileInputStream(foundFile));
					byte[] b = new byte[bis.available()];
					bis.read(b);
					contentBase64 = new BASE64Encoder().encode(b);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public Element createElement(SVGHandle handle, Rectangle2D bounds) {
		...
		// getting the image file path for the element
		String contentBase64 = this.getImageBase64(handle);

		if (contentBase64 != null && !contentBase64.equals("")) {
			...
			element.setAttributeNS(EditorToolkit.xmlnsXLinkNS, "xlink:href",
					"data:image/" + fileType + ";base64," + contentBase64);
			element.setAttributeNS(null, preserveAtt, "none meet");
			element.setAttributeNS(null, Constants.NCI_SVG_INNER_IMAGE_ATTR,
					Constants.NCI_SVG_INNER_IMAGE_VALUE);
			Toolkit.clearBatikImageCache();
			handle.getCanvas().clearCache();
