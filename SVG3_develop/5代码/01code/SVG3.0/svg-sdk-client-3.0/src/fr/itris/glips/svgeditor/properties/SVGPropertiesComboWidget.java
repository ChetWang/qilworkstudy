 ...
 */
public class SVGPropertiesComboWidget extends SVGPropertiesWidget {

    /**
     * the constructor of the class
     * @param propertyItem a property item
     */
	public SVGPropertiesComboWidget(SVGPropertyItem propertyItem) {
	...
	protected void buildComponent(){
		...
		final EditorAdapter editor=propertyItem.getProperties().getEditor();
		...
		String cur=null, propertyValue=propertyItem.getGeneralPropertyValue(), value="";
			
		//an item with an empty string
		item=new SVGPropertiesComboItem("","");
		itemList.add(item);
		...
		for(Iterator it=propertyItem.getPropertyValuesMap().keySet().iterator(); it.hasNext();){
			...
			try{
				cur=(String)it.next();
				value=(String)propertyItem.getPropertyValuesMap().get(cur);
