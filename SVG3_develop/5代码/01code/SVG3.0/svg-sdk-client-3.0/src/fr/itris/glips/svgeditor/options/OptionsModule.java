...
/**
 * the class of the module that provides widgets so that the user can
 * graphically change the square mode
 * 
 * @author Jordi SUC
 */
public class OptionsModule extends ModuleAdapter {
	 ...
	 * 暂时屏蔽掉闭合模式（ClosePathMode）,强力线模式（ConstraintLinesMode）
	 */
	// protected String[] ids={"RemanentMode", "SquareMode",
	// "AlignWithRulersMode",
	 ...
	 *            the editor
	 */
	public OptionsModule(EditorAdapter editor) {
		super(editor);
			...
			listener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (e.getSource() instanceof JCheckBoxMenuItem) {
						...
						optionsToolItems[index]
								.setSelected(optionsMenuItems[index]
										.isSelected());
						...
						optionsMenuItems[index]
								.setSelected(optionsToolItems[index]
										.isSelected());
	...
	protected void initializeItemsState() {
		...
		optionsMenuItems[0].setSelected(editor.getRemanentModeManager()
				.isRemanentMode());
		optionsMenuItems[1].setSelected(editor.getSquareModeManager()
				.isSquareMode());
		optionsMenuItems[2].setSelected(editor.getHandlesManager()
				.getRulersParametersHandler().alignWithRulers());

		optionsToolItems[0].setSelected(editor.getRemanentModeManager()
				.isRemanentMode());
		optionsToolItems[1].setSelected(editor.getSquareModeManager()
				.isSquareMode());
		optionsToolItems[2].setSelected(editor.getHandlesManager()
				.getRulersParametersHandler().alignWithRulers());
	...
	protected void doAction(int index, boolean isSelected) {
		...
		case 0:
			...
			editor.getRemanentModeManager().setRemanentMode(
					optionsMenuItems[index].isSelected());
			break;

		case 1:

			editor.getSquareModeManager().setSquareMode(
					optionsMenuItems[index].isSelected());
			break;

		case 2:

			editor.getHandlesManager().getRulersParametersHandler()
