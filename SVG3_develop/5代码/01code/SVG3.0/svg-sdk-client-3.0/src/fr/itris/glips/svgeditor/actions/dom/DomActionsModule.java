...
public abstract class DomActionsModule extends ModuleAdapter {
	...
	public DomActionsModule(EditorAdapter editor) {
		super(editor);
	}
	...
	protected void createItems() {
		...
		for (int i = 0; i < actionsIds.length; i++) {
			...
			// getting the labels
			actionsLabels[i] = ResourcesManager.bundle.getString(actionsIds[i]
					+ "ItemLabel");
			undoRedoActionsLabels[i] = ResourcesManager.bundle
					.getString(actionsIds[i] + "UndoRedoLabel");
	...
	public void initialize() {
		...
		Utilities.executeRunnable(new Runnable() {
			public void run() {
				while (moduleMenu.getParent() == null) {
					try {
						editor.getLogger().log(editor, LoggerAdapter.DEBUG, "DomActionsModule等待moduleMenu Parent");
						Thread.currentThread().sleep(10);						
					} catch (InterruptedException e) {
					}
				}
				moduleMenu.getParent().addPropertyChangeListener(
	...
	protected void updateMenuItems() {
		...
		if (handle == null
				...
				|| (handle.getSelection() != null && handle.getSelection()
						.isSelectionLocked())) {
			...
			Set<Element> selectedElements = handle.getSelection()
					.getSelectedElements();
	...
	public Collection<PopupItem> getPopupItems() {
		...
		PopupSubMenu subMenu = new PopupSubMenu(editor, moduleId, moduleLabel,
				moduleId);
		...
		for (int i = 0; i < actionsIds.length; i++) {
			...
			popupItem = new PopupItem(editor, actionsIds[i], actionsLabels[i],
					actionsIds[i]) {
				...
				public JMenuItem getPopupItem(LinkedList<Element> nodes) {
					...
					if (currentHandle != null
							&& selectionCorrect(index, elementsSet)) {
						...
						if (moduleId.equals("CanvasSize")) {
							if (editor.getHandlesManager().getCurrentHandle()
									.getSelection().getSelectedElements()
									.size() == 0) {
	 ...
	 *            a svg handle
	 * @param elements
	 *            a set of elements to be modified
	 * @param index
	 *            the index of the action
	 * @param evt
	 *            the event that triggered the action
	 */
	protected abstract void doAction(SVGHandle handle, Set<Element> elements,
	 ...
	 */
	protected void handleItemsState(SVGHandle handle,
			Set<Element> selectedElements) {

		for (int i = 0; i < actionsMenuItems.length; i++) {

			actionsMenuItems[i]
					.setEnabled(selectionCorrect(i, selectedElements));
		}
	}

	/**
	 * whether the provided elements (that are the currently selected elements),
	 * denote a correct selection for the module
	 * 
	 * @param index
	 *            the index defining the type of the action
	 * @param elements
	 *            the set of the currently selected elements
	 * @return whether the provided elements denote a correct selection for the
	 *         module
	 */
	...
	/**
	 * whether the provided elements (that are the currently selected elements),
	 * denote a correct selection for the module, FIRST TYPE IMPLEMENTATION
	 * 
	 * @param elements
	 *            the set of the currently selected elements
	 * @return whether the provided elements denote a correct selection for the
	...
	/**
	 * whether the provided elements (that are the currently selected elements),
	 * denote a correct selection for the module, SECOND TYPE IMPLEMENTATION
	 * 
	 * @param elements
	 *            the set of the currently selected elements
	 * @return whether the provided elements denote a correct selection for the
	...
	/**
	 * whether the provided elements (that are the currently selected elements),
	 * denote a correct selection for the module, THIRD TYPE IMPLEMENTATION
	 * 
	 * @param elements
	 *            the set of the currently selected elements
	 * @return whether the provided elements denote a correct selection for the
	 ...
	 * @param index
	 *            the index of the type of the action
	 * @param elementsToTranslationFactors
	 *            the map associating an element to its translation factors
	 */
	protected void applyTranslateTransform(SVGHandle handle, int index,
		...
		final Set<UndoRedoAction> undoRedoActions = new HashSet<UndoRedoAction>();
		...
		for (Element element : elementsToTranslationFactors.keySet()) {
			...
			shapeModule = ShapeToolkit.getShapeModule(element, editor);
			...
			if (shapeModule != null) {
				...
				if (translationFactors != null) {
					...
					undoRedoAction = shapeModule.translate(handle, elements,
		...
		if (undoRedoActions.size() > 0) {
			...
			addUndoRedoAction(handle, index, executeRunnable, undoRunnable,
	 ...
	 * @param index
	 *            the index of the type of the action
	 * @param elementsToTransform
	 *            the map associating an element to its transform
	 */
	protected void applyResizeTransform(SVGHandle handle, int index,
		...
		for (Element element : elementsToTransform.keySet()) {
			...
			if (shapeModule != null) {
				...
				if (transform != null) {
					...
					undoRedoAction = shapeModule.resize(handle, elements,
							transform);
	...
	/**
	 * applies the rotate transform denoted by the provided parameters to the
	 * provided elements
	 * 
	 * @param handle
	 *            the current svg handle
	 * @param index
	 *            the index of the type of the action
	 * @param elementsToCenter
	 *            the map associating an element to its center point for the
	 *            rotation
	 * @param elementsToAngle
	 *            the map associating an element to its angle for the rotation
	 */
	protected void applyRotateTransform(SVGHandle handle, int index,
		...
		for (Element element : elementsToCenter.keySet()) {
			...
			if (shapeModule != null) {
				...
				if (centerPoint != null) {
					...
					undoRedoAction = shapeModule.rotate(handle, elements,
							centerPoint, angle);
	 ...
	 * @param index
	 *            the index of the type of the action
	 * @param executeRunnable
	 *            the runnable for the execution
	 * @param undoRunnable
	 *            the runnable for undoing the action
	 * @param elements
	 *            the set of the elements to be modified
	 */
	protected void addUndoRedoAction(SVGHandle handle, int index,
			Runnable executeRunnable, Runnable undoRunnable,
		...
		UndoRedoActionList actionlist = new UndoRedoActionList(
				undoRedoActionsLabels[index], false);
	 ...
	 */
	protected Map<Element, Rectangle2D> getBounds(SVGHandle handle,
			Set<Element> elements) {

		// getting the bounds of each element
		Map<Element, Rectangle2D> elementsToBounds = new HashMap<Element, Rectangle2D>();

		Rectangle2D bounds = null;

		for (Element element : elements) {

			bounds = handle.getSvgElementsManager().getSensitiveBounds(element);
	...
	protected Rectangle2D union(Set<Rectangle2D> rectSet) {
		...
		for (Rectangle2D rect : rectSet) {
			...
			if (unionRect == null) {
				...
				unionRect = new Rectangle2D.Double(rect.getX(), rect.getY(),
	...
	/**
	 * the class of the comparator used to compare two elements according to
	 * their bounds
	 * 
	 * @author Jordi SUC
	 */
	protected class ElementComparator implements Comparator<Element> {
		 ...
		 *            the map associating an element to its bounds
		 * @param isHorizontal
		 *            whether the elements should be compared according to their
		 *            abscisses or according to their ordinate
		 */
		protected ElementComparator(
				Map<Element, Rectangle2D> elementToBoundsMap,
