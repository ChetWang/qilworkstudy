...
public class DistributeModule extends DomActionsModule {
	 ...
	 */
	public DistributeModule(EditorAdapter editor) {
	...
	protected double getLeftSpace(Collection<Rectangle2D> elementsBounds, 
        ...
        if(isHorizontal)
            leftSpace = unionRect.getWidth();
        else
            leftSpace = unionRect.getHeight();
		for(Rectangle2D bounds : startSet){
		    if(isHorizontal)
		    {
		        leftSpace -= bounds.getWidth();
		    }
		    else
		    {
		        leftSpace-= bounds.getHeight();
		    }
    	}
