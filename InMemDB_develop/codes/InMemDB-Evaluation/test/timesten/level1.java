...
public class level1 {
	...
	/**
	 * Prefix URL to pass to DriverManager.getConnection() for TimesTen Direct
	 * Connection
	 */
	private static final String directURLPrefix = "jdbc:timesten:direct:";

	/**
	 * Prefix of URL to pass to DriverManager.getConnection() for TimesTen
	 * client Connection
	 */
	...
	/**
	 * Flag indicating whether the shutdown hook thread should wait and hold-up
	 * the JVM exit so that the running examples can implement a clean exit
	 * first. This flag is set only when the examples are being run.
	 */
	static boolean shouldWait = false;

	/**
	 * A synchronization object used by the shutdown hook thread to wait for an
	 * orderly database disconnect before exiting.
	 */
	static Object stopMonitor;

	/**
	 * Flag indicating whether a "signal" has been received. This flag is set by
	 * our shutdown hook thread.
	 */
	 ...
	 * 
	 * 1. Parse the arguments. 2. Instantiate and run the level1 example.
	 */
	public static void main(String[] args) {
		...
		try {
			...
			retcode = lvl1.runexample(myLib.opt_doTrace, myLib.opt_doClient,
					myLib.opt_connstr);
...
//			String password = ac.getPassword();
			...
			String username = "qil.wong";
			String password = "anan1303";
			...
			if (ex.getSQLState().equalsIgnoreCase("S0002")) {
				...
				// native Error 2206 TT_ERR_TABLE_DOES_NOT_EXIST. Since these
				// two
				// errors return equivalent information, we match against
				// SQLState
				...
				// types of constraint violations that return the same SQLState,
				// we
	...
	/**
	 * Check if a "signal" is pending - i.e. the shutdown hook thread has been
	 * entered.
	 * 
	 ...
	 */
	private static boolean CheckIfStopIsRequested() throws InterruptedException {
