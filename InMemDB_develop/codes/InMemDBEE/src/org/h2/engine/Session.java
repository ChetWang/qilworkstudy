...
public class Session extends SessionWithState implements SessionFactory,
	...
	public void commit(boolean ddl) {
		...
		// add by Qil.Wong, 这种情况下需要群发集群消息
		sendCommitRollbackNote(CommitRollbackNote.COMMIT);
	}
	...
	public void rollback() {
		...
		sendCommitRollbackNote(CommitRollbackNote.ROLLBACK);
	}

	private void sendCommitRollbackNote(int type) {
		sendClusterCommitRollbackNote(type);
		sendBackupCommitRollbackNote(type);
		InMemDBServer.getInstance().getIdleMasterDataBaseSessions().remove(id);
	}

	/**
	 * 实物一致性下，发送集群消息 added by qil.wong
	 * 
	 * @param type
	 */
	private void sendClusterCommitRollbackNote(int type) {
		if (InMemDBServer.getInstance().getIdleMasterDataBaseSessions().get(id) != null
				&& !this.isSlave()) {
			DBLogger.log(DBLogger.DEBUG, "发送集群commit/rollback消息:" + type
					+ " Session ID = " + id);
			ClusterIfc cluster = (ClusterIfc) InMemDBServer.getInstance()
					.getModule(ClusterIfc.class);
			if (cluster != null) {
				CommitRollbackNote crNote = new CommitRollbackNote(id, type);
				cluster.sendDataToMembers(crNote, null);
			}
		}
	}

	/**
	 * 事务一致性下的备份操作 added by qil.wong
	 * 
	 * @param type
	 */
	private void sendBackupCommitRollbackNote(int type) {
		if (InMemDBServer.getInstance().getIdleMasterDataBaseSessions().get(id) != null
				&& !this.isSlave()) {
			PersistentBackupRestoreIfc backup = (PersistentBackupRestoreIfc) InMemDBServer
					.getInstance().getModule(PersistentBackupRestoreIfc.class);
			if (backup != null) {
				CommitRollbackNote crNote = new CommitRollbackNote(id, type);
				backup.insertSignal(crNote);
			}
		}
	}
